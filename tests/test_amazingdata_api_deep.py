...
def validate_response(name: str, endpoint: str, response: requests.Response, 
                      check_data_exists: bool = True) -> dict:
    """
    验证响应数据
        ...
        if isinstance(actual_data, list):
            ...
            if len(actual_data) > 0:
                ...
                if isinstance(first_item, dict):
                    result["data_summary"] = f"列表[{len(actual_data)}条], 首条字段: {list(first_item.keys())[:8]}"
                else:
                    result["data_summary"] = f"列表[{len(actual_data)}条], 首项: {str(first_item)[:50]}"
            else:
            ...
            result["data_summary"] = f"类型: {type(actual_data).__name__}, 值: {str(actual_data)[:100]}"
...
def test_api(name: str, endpoint: str, method: str = "GET", 
    ...
    try:
        ...
        results["details"].append({
            "name": name, "endpoint": endpoint, "passed": False,
            "reason": "请求超时", "status_code": 0, "record_count": 0
        })
    except requests.exceptions.ConnectionError:
        results["failed"] += 1
        log(f"  [FAIL] 连接失败，服务未启动?")
        results["details"].append({
            "name": name, "endpoint": endpoint, "passed": False,
            "reason": "连接失败", "status_code": 0, "record_count": 0
        })
        ...
        results["details"].append({
            "name": name, "endpoint": endpoint, "passed": False,
            "reason": str(e), "status_code": 0, "record_count": 0
        })
...
def run_all_tests():
    ...
    with open(LOG_FILE, "w", encoding="utf-8") as f:
        f.write(f"AmazingData API 深度测试 v2\n")
    ...
    # GET 接口
    test_api("get_code_info", "/basic/code-info", 
             method="GET",
             params={"security_type": "EXTRA_STOCK_A"})
    
    test_api("get_calendar", "/basic/calendar",
             method="GET",
             params={"market": "SH", "begin_date": 20240101, "end_date": 20240131})
    
    test_api("get_code_list", "/basic/code-list",
             method="GET",
             params={"security_type": "EXTRA_STOCK_A"})
    
    test_api("get_future_code_list", "/basic/future-code-list",
             method="GET")
    
    test_api("get_bj_code_mapping", "/basic/bj-code-mapping",
             method="GET")
    
    # POST 接口
    test_api("get_stock_basic", "/basic/stock-basic",
             method="POST",
             json_body={"code_list": ["SH.600000", "SZ.000001"]})
    
    test_api("get_backward_factor", "/basic/backward-factor",
             method="POST",
             json_body={
                 "code_list": ["SH.600000"],
                 "begin_date": 20240101,
    ...
    test_api("get_adj_factor", "/basic/adj-factor",
             method="POST",
             json_body={
                 "code_list": ["SH.600000"],
                 "begin_date": 20240101,
    ...
    test_api("get_history_stock_status", "/basic/history-stock-status",
             method="POST",
             json_body={
                 "code_list": ["SH.600000"],
                 "begin_date": 20240101,
    ...
    test_api("get_hist_code_list", "/basic/hist-code-list",
             method="POST",
             json_body={
    ...
    test_api("get_balance_sheet", "/financial/balance-sheet",
             method="POST",
             json_body={
                 "code_list": ["SH.600000"],
                 "report_type": "quarter",
    ...
    test_api("get_cash_flow", "/financial/cash-flow",
             method="POST",
             json_body={
                 "code_list": ["SH.600000"],
                 "report_type": "quarter",
    ...
    test_api("get_income", "/financial/income",
             method="POST",
             json_body={
                 "code_list": ["SH.600000"],
                 "report_type": "quarter",
    ...
    test_api("get_profit_express", "/financial/profit-express",
             method="POST",
             json_body={
                 "code_list": ["SH.600000"],
                 "is_local": True
    ...
    test_api("get_profit_notice", "/financial/profit-notice",
             method="POST",
             json_body={
                 "code_list": ["SH.600000"],
                 "is_local": True
    ...
    test_api("get_financial_summary", "/financial/financial-summary",
             method="POST",
    ...
    test_api("get_margin_summary", "/margin/margin-summary",
             method="GET")
    
    test_api("get_margin_detail", "/margin/margin-detail",
             method="POST",
             params={"code": "SH.600000"})
    
    test_api("get_long_hu_bang", "/margin/long-hu-bang",
             method="POST",
             params={"code": "SH.600000"})
    
    test_api("get_block_trading", "/margin/block-trading",
             method="POST",
             params={"code": "SH.600000"})
