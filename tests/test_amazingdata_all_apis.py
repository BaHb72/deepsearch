...
async def provider(mock_ad):
    ...
    with patch('deepsearch.infrastructure.providers.implementations.amazingdata.amazingdata_extended.ad', mock_ad):
        with patch('deepsearch.infrastructure.providers.implementations.amazingdata.amazingdata_extended.HAS_AMAZINGDATA', True):
...
class TestBasicData:
    ...
    async def test_get_adj_factor(self, provider):
        """测试3.5.2.5 单次复权因子"""
        provider._base_data.get_adj_factor.return_value = pd.DataFrame({
            '000001.SZ': [1.0, 1.05]
        })
    ...
    async def test_get_stock_basic(self, provider):
        """测试3.5.2.8 证券基础信息"""
        provider._info_data.get_stock_basic.return_value = pd.DataFrame({
            'LISTDATE': [20100101]
        })
...
class TestFinancialData:
    ...
    async def test_get_cash_flow(self, provider):
        """测试3.5.5.2 现金流量表"""
        provider._info_data.get_cash_flow.return_value = pd.DataFrame({
            'NET_PROFIT': [100000]
        })
    ...
    async def test_get_income(self, provider):
        """测试3.5.5.3 利润表"""
        provider._info_data.get_income.return_value = pd.DataFrame({
            'TOT_OPERA_REV': [500000]
        })
    ...
    async def test_get_profit_notice(self, provider):
        """测试3.5.5.5 业绩预告"""
        provider._info_data.get_profit_notice.return_value = pd.DataFrame({
            'PROFIT_CHANGE': [50.0]
        })
...
class TestShareholderData:
    ...
    async def test_get_holder_num(self, provider):
        """测试3.5.6.2 股东人数"""
        provider._info_data.get_holder_num.return_value = pd.DataFrame({
            'HOLDER_NUM': [10000]
        })
...
class TestShareholderRights:
    ...
    async def test_get_dividend(self, provider):
        """测试3.5.7.1 分红数据"""
        provider._info_data.get_dividend.return_value = pd.DataFrame({
            'DIVIDEND': [0.5]
        })
    ...
    async def test_get_right_issue(self, provider):
        """测试3.5.7.2 配股数据"""
        provider._info_data.get_right_issue.return_value = pd.DataFrame({
            'ISSUE_PRICE': [8.0]
        })
...
class TestMarginTrading:
    ...
    async def test_get_margin_detail(self, provider):
        """测试3.5.8.2 融资融券明细"""
        provider._info_data.get_margin_detail.return_value = pd.DataFrame({
            'MARGIN_BUY': [100000]
        })
...
class TestMarketAnomaly:
    ...
    async def test_get_long_hu_bang(self, provider):
        """测试3.5.9.1 龙虎榜"""
        provider._info_data.get_long_hu_bang.return_value = pd.DataFrame({
            'BUY_AMT': [10000000]
        })
