...
class AmazingDataLoginSimulator:
    ...
    def __init__(self):
        # 从配置文件中提取的服务器信息
        self.servers = [
            {"name": "电信服务器1", "host": "101.230.159.234", "port": 8600, "network": "telecom"},
            {"name": "电信服务器2", "host": "120.86.124.106", "port": 8600, "network": "telecom"},
            {"name": "联通服务器", "host": "140.206.44.234", "port": 8600, "network": "unicom"},
        ]
    ...
    def test_server_connectivity(self, server):
        ...
        try:
            ...
            if result == 0:
                ...
                sock.close()
                return {"success": True, "elapsed_time": elapsed_time, "message": "端口连接成功"}
            else:
            ...
            return {"success": False, "elapsed_time": elapsed_time, "message": "连接超时"}
    ...
    def print_summary(self):
        ...
        logger.info("-" * 60)
        logger.info(
            f"{'服务器':<15} {'网络':<10} {'连接':<8} {'登录':<8} {'耗时(秒)':<10} {'备注'}"
        )
        logger.info("-" * 60)
