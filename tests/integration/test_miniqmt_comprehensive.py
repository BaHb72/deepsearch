...
class MiniQMTCapabilityTester:
    ...
    def test_connection(self) -> TestCategory:
        ...
        category = TestCategory(
            name="连接测试",
            description="测试 xtdata 模块导入和 MiniQMT 连接"
        )
        ...
        def test_import():
            try:
                ...
                # 检查关键函数
                required_funcs = [
                    "get_full_tick", "get_market_data", "download_history_data",
                    "get_instrument_detail", "get_sector_list", "get_trading_dates"
                ]
    ...
    def test_basic_info(self) -> TestCategory:
        ...
        category = TestCategory(
            name="基础信息测试",
            description="测试合约信息、板块、交易日历等基础数据"
        )
        ...
        category.results.append(self.run_test("合约详情 (get_instrument_detail)", test_instrument_detail))
        ...
        category.results.append(self.run_test("板块成分股 (get_stock_list_in_sector)", test_sector_stocks))
        
        # 测试 4.4: 交易日历
        def test_trading_dates():
            result = self.xtdata.get_trading_dates("SH", start_time=self.start_date, end_time=self.end_date)
            if result:
                count = len(result) if isinstance(result, list) else 0
            ...
            try:
                ...
                if result:
                    ...
                    if self.test_etf in result:
                        ...
                        return True, f"ETF {self.test_etf} 信息获取成功, 包含 {len(fields)} 个字段", etf_data
        ...
        def test_index_weight():
            try:
                ...
                if result:
                    ...
                    return True, f"指数 {self.test_index} 包含 {count} 只成分股, 示例: {sample}", result
                return True, f"指数权重数据为空 (可能需要交易时间或投研版)", None
            except AttributeError:
            ...
            try:
                ...
                if result is not None:
                    if hasattr(result, 'empty'):
                        ...
                        return True, f"复权因子获取成功 (dict格式)", result
    ...
    def test_subscription(self) -> TestCategory:
        ...
        def test_subscribe_tick():
            ...
            try:
                ...
                if seq_id and seq_id > 0:
                    ...
                    return True, f"订阅测试完成，订阅号={seq_id}，收到 {len(received_data)} 条数据", received_data
        ...
        def test_subscribe_1m():
            ...
            try:
                ...
                if seq_id and seq_id > 0:
                    self.xtdata.unsubscribe_quote(seq_id)
                    return True, f"1分钟订阅测试完成，订阅号={seq_id}，收到 {len(received_data)} 条数据", received_data
    ...
    def test_performance(self) -> TestCategory:
        ...
        def test_batch_latency():
            # 准备 20 只股票
            stocks = [f"00000{i}.SZ" for i in range(1, 10)] + [f"60000{i}.SH" for i in range(0, 10)][:11]
        ...
        def test_throughput():
            ...
            return True, f"{request_count} 次请求耗时 {total_time:.2f}s, 吞吐量: {throughput:.1f} req/s", None
    ...
    def test_cb_and_ipo(self) -> TestCategory:
        ...
        category.results.append(self.run_test("下载可转债数据 (download_cb_data)", test_download_cb))
    ...
    def test_instrument_extended(self) -> TestCategory:
        ...
        category = TestCategory(
            name="合约扩展信息测试",
            description="测试合约类型、完整合约信息、交易日历等扩展功能"
        )
        ...
        category.results.append(self.run_test("获取合约类型 (get_instrument_type)", test_instrument_type))
        ...
        category.results.append(self.run_test("获取完整合约信息 (iscomplete=True)", test_full_instrument))
        ...
        category.results.append(self.run_test("交易日历 (get_trading_calendar)", test_trading_calendar))
        ...
        category.results.append(self.run_test("下载过期合约 (download_history_contracts)", test_download_history_contracts))
    ...
    def test_batch_and_whole_quote(self) -> TestCategory:
        ...
        category.results.append(self.run_test("批量下载历史数据 (download_history_data2)", test_download_history_data2))
        ...
        def test_subscribe_whole_quote():
            ...
            try:
                ...
                return True, f"全推订阅测试完成, 订阅号={seq}, 回调 {len(received_data)} 次", received_data
            except AttributeError:
                return True, "subscribe_whole_quote 方法不存在", None
            except Exception as e:
                return True, f"全推订阅跳过: {e}", None
        
        category.results.append(self.run_test("订阅全推行情 (subscribe_whole_quote)", test_subscribe_whole_quote))
        ...
        category.results.append(self.run_test("获取除权数据 (get_divid_factors)", test_divid_factors_fixed))
    ...
    def run_all_tests(self) -> Dict[str, Any]:
        ...
        # 打印汇总
        self.print_summary(total_tests, passed_tests, failed_tests, skipped_tests, error_tests, total_time)
    ...
    def print_summary(self, total: int, passed: int, failed: int, skipped: int, error: int, duration: float):
        """打印测试汇总"""
        ...
        if self.console:
            ...
            table.add_row("[green]通过[/green]", str(passed), f"{passed/total*100:.1f}%" if total else "0%")
            table.add_row("[red]失败[/red]", str(failed), f"{failed/total*100:.1f}%" if total else "0%")
            table.add_row("[yellow]跳过[/yellow]", str(skipped), f"{skipped/total*100:.1f}%" if total else "0%")
            table.add_row("[red]错误[/red]", str(error), f"{error/total*100:.1f}%" if total else "0%")
