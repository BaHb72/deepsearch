...
class FakeAmazingDataProvider:
    ...
    async def get_calendar(self, data_type="int", market="SH"):
        days = [20251101, 20251104, 20251105]
        if data_type == "str":
...
async def test_factory_service_refreshes_board_universe():
    ...
    await service.ensure_subscription(["测试"])
...
async def test_streaming_pipeline_with_config_overrides():
    ...
    realtime_conf = MarketRealtimeConfig(
        ...
        capital_limit=5,
        order_limit=7,
        initial_step_timeout_seconds=8.0,
        auction_interval_seconds=4.0,
        auction_timeout_seconds=4.5,
        off_day_interval_seconds=240.0,
    ...
    assert list(pipeline.boards) == ["测试", "创业板"]
    assert runner.interval_seconds == 2.0
    assert runner.initial_step_timeout_seconds == 8.0
    ...
    assert runner.session_guard.phase_intervals[PhaseState.OFF_DAY] == 240.0
    assert runner.session_guard.phase_intervals[PhaseState.AUCTION] == 4.0
    assert runner.session_guard.phase_timeouts[PhaseState.AUCTION] == 4.5
    assert runner.session_guard.markets == ("SH", "SZ", "BJ")
