...
class TestRealtimeQuotes:
    ...
    async def test_get_multiple_quotes(self, test_data_provider):
        ...
        for symbol in symbols:
            test_data_provider.get_realtime_quote = AsyncMock(
                return_value={"symbol": symbol, "price": 10.0 + len(symbol), "change": 0.5}
            )
            quote = await test_data_provider.get_realtime_quote(symbol)
            quotes.append(quote)
...
class TestOrderBook:
    ...
    async def test_get_level2_orderbook(self):
        ...
        provider = AsyncMock()
        provider.get_orderbook = AsyncMock(
            return_value={
                "symbol": "000001",
...
class TestMarketIndices:
    ...
    async def test_get_major_indices(self):
        ...
        provider = AsyncMock()
        provider.get_indices = AsyncMock(
            return_value=[
                {
                    "code": "000001",
                    "name": "上证指数",
    ...
    async def test_index_historical_data(self):
        ...
        provider = AsyncMock()
        provider.get_index_history = AsyncMock(
            return_value=[
...
class TestHotStocks:
    ...
    async def test_get_hot_stocks(self):
        ...
        provider = AsyncMock()
        provider.get_hot_stocks = AsyncMock(
            return_value=[
                {
                    "symbol": "000001",
                    "name": "平安银行",
    ...
    async def test_get_limit_up_stocks(self):
        ...
        provider = AsyncMock()
        provider.get_limit_stocks = AsyncMock(
            return_value={
                "limit_up": [
                    {"symbol": "000001", "name": "平安银行", "time": "09:35:00"},
                    {"symbol": "000002", "name": "万科A", "time": "10:15:00"},
                ],
                "limit_down": [{"symbol": "600001", "name": "邯郸钢铁", "time": "09:31:00"}],
            }
        )
