...
class BenchmarkRunner:
    ...
    def compare_results(self, before: BenchmarkSuite, after: BenchmarkSuite) -> Dict[str, Any]:
        ...
        for name in before_map:
            if name in after_map:
                ...
                comparison[name] = {
                    'throughput_change': (after_result.throughput - before_result.throughput) / before_result.throughput * 100,
                    'latency_p50_change': (after_result.latency_p50 - before_result.latency_p50) / before_result.latency_p50 * 100,
                    'latency_p95_change': (after_result.latency_p95 - before_result.latency_p95) / before_result.latency_p95 * 100,
                    'memory_change': (after_result.memory_used - before_result.memory_used) / before_result.memory_used * 100 if before_result.memory_used > 0 else 0,
                }
    ...
    def save_results(self, suite: BenchmarkSuite):
        ...
        with open(csv_file, 'w', newline='') as f:
            writer = csv.writer(f)
            writer.writerow(['Name', 'Throughput', 'Latency P50', 'Latency P95', 'Latency P99', 'Memory (MB)', 'CPU %'])
            for r in suite.results:
                writer.writerow([
                    r.name,
                    f"{r.throughput:.2f}",
