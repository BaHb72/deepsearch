...
def _create_source_stats() -> SourceStats:
    return {"success": 0, "error": 0}


def _create_monitoring_stats() -> MonitoringStats:
    return {
        "total_records": 0,
        "success_count": 0,
        "error_count": 0,
        "by_source": cast(Dict[str, SourceStats], {}),
        "by_operation": cast(Dict[str, int], {}),
...
class StructuredMonitorLogger:
    ...
    _instance: ClassVar[Optional["StructuredMonitorLogger"]] = None
    _lock: ClassVar[Lock] = Lock()
    _initialized: ClassVar[bool] = False

    def __new__(cls) -> "StructuredMonitorLogger":
    ...
    def __init__(self) -> None:
        ...
        # 日志输出路径
        self.log_dir: Path = Path("data/logs/datasources")
        self.log_dir.mkdir(parents=True, exist_ok=True)

        # 当前日志文件
        self.current_log_file: Path = self._get_log_file_path()

        # 使用 loguru 添加监控日志文件输出，启用轮转、保留和压缩
        # - rotation="00:00": 每天午夜轮转
        # - retention="30 days": 保留30天
        # - compression="gz": 使用gzip压缩归档日志
        self._monitor_logger_id = logger.add(
            str(self.log_dir / "monitor_{time:YYYYMMDD}.jsonl"),
            format="{message}",  # 纯消息格式，因为我们输出的是 JSON
            rotation="00:00",  # 每天午夜轮转
            retention="30 days",  # 保留30天
            compression="gz",  # gzip压缩归档
            encoding="utf-8",
            filter=lambda record: record.get("extra", {}).get("monitor_log", False),  # type: ignore[arg-type]
            level="INFO",
        )

        # 监控数据导出路径
        self.export_dir: Path = Path("data/monitoring/exports")
        ...
        # 统计信息
        self.stats: MonitoringStats = _create_monitoring_stats()
        ...
        logger.info("结构化监控日志系统初始化完成（启用日志轮转和压缩）")

    def _get_log_file_path(self) -> Path:
        """获取当前日志文件路径"""
        date_str = datetime.now().strftime("%Y%m%d")
        return self.log_dir.joinpath(f"monitor_{date_str}.jsonl")
    ...
    def log(self, record: MonitoringRecord) -> None:
        """记录监控日志"""
    ...
    def _update_stats(self, record: MonitoringRecord) -> None:
        """更新统计信息"""
        ...
        if record.source_type:
            ...
            source_stats = self.stats["by_source"].get(source_key)
            if source_stats is None:
                source_stats = _create_source_stats()
                self.stats["by_source"][source_key] = source_stats
        ...
        self.stats["by_operation"][op_key] = self.stats["by_operation"].get(op_key, 0) + 1
    ...
    def flush(self) -> None:
        ...
        with self.buffer_lock:
            ...
            # 使用 loguru 记录日志（利用内置轮转和压缩）
            try:
                monitor_sink = logger.bind(monitor_log=True)
                for record in self.buffer:
                    monitor_sink.info(record.to_json())
    ...
    def export_stats(self) -> None:
        ...
        export_data: ExportStatsPayload = {
            "timestamp": time.time(),
    ...
    def _get_recent_errors(self, limit: int = 100) -> List[Dict[str, Any]]:
        """获取最近的错误记录"""
    ...
    def log_http_request(
        ...
        **kwargs: Any,
        ...
        # 设置元数据
        record.metadata = {"url": url, "method": method, "status_code": status_code, **kwargs}
    ...
    def log_database_query(
        ...
        **kwargs: Any,
        ...
        """记录数据库查询"""
        record = MonitoringRecord(operation=OperationType.DATABASE_QUERY, success=error is None)

        record.metadata = {"query": query[:500], **kwargs}  # 限制查询长度
