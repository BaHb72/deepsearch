...
from core.observability.analyzers.error_analyzer import get_error_analyzer
from core.observability.logging.monitoring_logger import (
    ErrorInfo,
    ErrorType,
    MonitoringRecord,
    OperationType,
...
from core.observability.metrics.collectors.metrics_collector import get_metrics_collector
...
def _ensure_latency_ms(value: Optional[float]) -> float:
    """统一处理可能为 None 的延迟值。"""
    if value is None:
        return 0.0
    return float(value)
...
def monitor_http_request(
    ...
    extract_url: Optional[Callable[..., Any]] = None,
    ...
    def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        async def async_wrapper(*args: Any, **kwargs: Any) -> Any:
            # 创建监控记录
            record = MonitoringRecord(source_type=source_type, operation=OperationType.HTTP_REQUEST)
            ...
            try:
                ...
                error_analyzer.analyze_error(
                    record.error, source_type.value if source_type else "unknown", record.metadata
                )
...
def monitor_database_query(
    ...
    def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
        @functools.wraps(func)
        async def async_wrapper(*args: Any, **kwargs: Any) -> Any:
            record = MonitoringRecord(operation=OperationType.DATABASE_QUERY)
        ...
        def sync_wrapper(*args: Any, **kwargs: Any) -> Any:
            ...
            try:
                ...
                record.success = False
                record.error = ErrorInfo(error_type=ErrorType.DATA_ERROR, error_message=str(e))
                raise
            finally:
...
def monitor_cache_operation(
    ...
    def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        def wrapper(*args: Any, **kwargs: Any) -> Any:
            record = MonitoringRecord(operation=OperationType.CACHE_ACCESS)
            ...
            try:
                ...
                record.success = False
                record.error = ErrorInfo(error_type=ErrorType.UNKNOWN_ERROR, error_message=str(e))
                raise
    ...
    def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
        ...
        async def async_wrapper(*args: Any, **kwargs: Any) -> Any:
            record = MonitoringRecord(
                source_type=DataSourceType.QMT, operation=OperationType.TCP_CONNECT
            )
...
def monitor_batch_process(
    ...
    def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
        @functools.wraps(func)
        async def async_wrapper(*args: Any, **kwargs: Any) -> Any:
            record = MonitoringRecord(operation=OperationType.BATCH_PROCESS)
            ...
            try:
                ...
                # 计算处理速率
                latency_ms = _ensure_latency_ms(record.performance.latency_ms)
                if "batch_size" in record.metadata and latency_ms > 0:
                    rate = record.metadata["batch_size"] / (latency_ms / 1000)
...
def monitor_circuit_breaker(
    ...
    def decorator(func: Callable[..., Any]) -> Callable[..., Any]:
        # 熔断器状态
        state: CircuitBreakerState = {"failures": 0, "last_failure_time": 0.0, "is_open": False}
