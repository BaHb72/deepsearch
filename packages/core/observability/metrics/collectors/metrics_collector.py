...
import numpy as np
from core.observability.logging.monitoring_logger import MonitoringRecord, OperationType
...
class _RealtimeAccumulator(TypedDict):
    success: int
    error: int
...
def _create_snapshot_buffer() -> Deque[PerformanceSnapshot]:
...
class MetricsCollector:
    ...
    def __init__(self) -> None:
        ...
        # 按数据源和操作类型分组的指标
        self.metrics_by_source: DefaultDict[str, Deque[PerformanceSnapshot]] = defaultdict(
            _create_snapshot_buffer
        )
        self.metrics_by_operation: DefaultDict[str, Deque[PerformanceSnapshot]] = defaultdict(
            _create_snapshot_buffer
        )
        ...
        # 实时统计（最近1分钟）
        self.realtime_window: Deque[PerformanceSnapshot] = deque(maxlen=1000)

        # 性能趋势分析
        self.trend_data: DefaultDict[str, List[Tuple[float, float]]] = defaultdict(list)
    ...
    def collect(self, record: MonitoringRecord) -> None:
        """收集监控记录"""
        latency_value = record.performance.latency_ms
    ...
    def _update_trend(self, snapshot: PerformanceSnapshot) -> None:
        """更新趋势数据"""
        ...
        self.trend_data[key] = [(ts, lat) for ts, lat in self.trend_data[key] if ts > cutoff_time]

    def _detect_anomalies(self, snapshot: PerformanceSnapshot) -> None:
        """检测性能异常"""
        # 获取历史平均值
        key = f"{snapshot.source_type.value if snapshot.source_type else 'unknown'}_{snapshot.operation.value}"
        ...
        if (
            snapshot.latency_ms
            > avg_latency + self.anomaly_threshold["latency_spike"] * std_latency
        ):
            logger.warning(
                f"检测到延迟异常: {key} - "
    ...
    def aggregate_metrics(self) -> None:
        ...
        # 定义时间窗口
        windows = [("1min", 60), ("5min", 300), ("1hour", 3600), ("24hour", 86400)]
        ...
        with self.lock:
            for window_name, window_seconds in windows:
                ...
                # 过滤时间窗口内的数据
                window_data = [m for m in self.raw_metrics if m.timestamp > cutoff_time]
                ...
                aggregated = self._create_aggregated_metrics(
                    window_name, cutoff_time, current_time, window_data
                )
    ...
    def _create_aggregated_metrics(
        self, window_name: str, start_time: float, end_time: float, data: List[PerformanceSnapshot]
    ) -> AggregatedMetrics:
        """创建聚合指标"""
        metrics = AggregatedMetrics(
            time_window=window_name, start_time=start_time, end_time=end_time
        )
        ...
        if request_sizes:
            metrics.avg_request_size = float(np.mean(request_sizes))
        if response_sizes:
            metrics.avg_response_size = float(np.mean(response_sizes))
    ...
    def get_realtime_metrics(self) -> Dict[str, MonitorSnapshot]:
        """获取实时指标（最近1分钟）"""
        with self.lock:
            ...
            recent_data = [m for m in self.realtime_window if m.timestamp > cutoff_time]

            if not recent_data:
                return {}

            # 按数据源分组统计
            def _new_source_stats() -> _RealtimeAccumulator:
                return {"success": 0, "error": 0, "latencies": []}

            by_source: Dict[str, _RealtimeAccumulator] = {}

            for snapshot in recent_data:
                key = snapshot.source_type.value if snapshot.source_type else "unknown"
                entry = by_source.get(key)
                if entry is None:
                    entry = _new_source_stats()
                    by_source[key] = entry
                if snapshot.success:
                    entry["success"] += 1
                else:
                    entry["error"] += 1
                entry["latencies"].append(float(snapshot.latency_ms))

            # 计算统计指标
            summaries: Dict[str, MonitorSnapshot] = {}
            for source, data in by_source.items():
                ...
                if latencies:
                    ...
                    avg_latency = 0.0
                    p95_latency = 0.0
                error_rate = float(data["error"] / total) if total else 0.0
                summaries[source] = {
                    "total": total,
    ...
    def get_performance_trend(
        self, source_type: Optional[str] = None, operation: Optional[str] = None, hours: int = 24
    ) -> List[Dict]:
        """获取性能趋势"""
        ...
        for key in keys:
            ...
            if data_points:
                result.append({"key": key, "data": data_points})
    ...
    def cleanup_old_data(self) -> None:
        ...
        with self.lock:
            ...
            for key in list(self.trend_data.keys()):
                self.trend_data[key] = [
                    (ts, lat) for ts, lat in self.trend_data[key] if ts > cutoff_time
                ]
