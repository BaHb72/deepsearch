...
def get_error_handler():
    from core.core.utils.error_handler import error_handler
...
def get_query_optimizer():
    from core.infrastructure.persistence.query_optimizer import query_optimizer
...
def errors(last, export):
    ...
    for error in error_history:
        ...
        table.add_row(
            timestamp, error_type, message, f"[{severity_color}]{severity}[/{severity_color}]"
        )
    ...
    if export:
        ...
        export_dir = logger_manager.ensure_subdirectory("errors")
        filepath = export_dir / f"error_report_{timestamp}.json"
...
def memory(top, cleanup, analyze):
    ...
    if large_objects:
        ...
        for obj in large_objects:
            table.add_row(obj["name"][:30], f"{obj['size_mb']:.2f}", obj["type"])
    ...
    if analyze:
        ...
        if trends := analysis.get("trends"):
            if trends.get("growing"):
                console.print("[yellow]⚠ 内存使用呈增长趋势[/yellow]")
        ...
        if recommendations := analysis.get("recommendations"):
            ...
            for rec in recommendations:
                level_color = {"WARNING": "yellow", "ERROR": "red", "INFO": "cyan"}.get(
                    rec["level"], "white"
                )
...
def monitor():
    ...
    layout["header"].update(
        Panel("[bold cyan]DeepSearch 实时监控[/bold cyan]", border_style="cyan")
    )
    ...
    def get_monitor_data():
        ...
        # 内存信息
        get_memory_manager().get_memory_info()
...
@debug.command()
@click.option(
    "--format", type=click.Choice(["json", "html", "text"]), default="json", help="报告格式"
)
def report(format):
    """生成完整调试报告"""
    ...
    debug_dir = logger_manager.ensure_subdirectory("debug")

    if format == "json":
        filepath = debug_dir / f"report_{timestamp}.json"

        with open(filepath, "w", encoding="utf-8") as f:
            json.dump(report_data, f, ensure_ascii=False, indent=2, default=str)

    elif format == "html":
        filepath = debug_dir / f"report_{timestamp}.html"
        ...
        filepath = debug_dir / f"report_{timestamp}.txt"

        with open(filepath, "w", encoding="utf-8") as f:
            f.write("DeepSearch 调试报告\n")
