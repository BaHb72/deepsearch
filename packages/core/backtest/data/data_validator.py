...
class DataValidator:
    ...
    def validate_time_series(self, df: pd.DataFrame, expected_freq: str = "D") -> Dict[str, Any]:
        ...
        if expected_freq == "D":
            # 创建完整的交易日期范围
            start_value = cast(Any, df.index[0])
            end_value = cast(Any, df.index[-1])
            ...
            if len(missing_dates) > 0:
                ...
                if missing_ratio > 0.3:  # 缺失超过30%
                    result["warnings"].append(
                        f"缺失 {len(missing_dates)} 个交易日 ({missing_ratio:.1%})"
                    )
        ...
        result["stats"] = {
            ...
            "avg_gap_days": (
                np.mean(np.diff(df.index.values).astype("timedelta64[D]").astype(int))
                if len(df) > 1
                else 0
            ),
        }
    ...
    def validate_volume(self, df: pd.DataFrame) -> Dict[str, Any]:
        ...
        if "volume" in df.columns:
            result["stats"] = {
                ...
                "volume_cv": (
                    df["volume"].std() / df["volume"].mean() if df["volume"].mean() > 0 else 0
                ),
            }
    ...
    def compare_data_sources(
        self, data_dict: Dict[str, pd.DataFrame], tolerance: float = 0.01
    ) -> Dict[str, Any]:
        """
        对比不同数据源的数据
        ...
        Args:
            ...
            tolerance: 价格差异容忍度（1%）
        """
        result: Dict[str, Any] = {"passed": True, "errors": [], "warnings": [], "comparison": {}}
        ...
        for compare_source in sources[1:]:
            ...
            for field in ["open", "high", "low", "close"]:
                if field in base_df.columns and field in compare_df.columns:
                    ...
                    if max_diff > tolerance:
                        ...
                        price_diffs.append(
                            {"field": field, "max_diff": max_diff, "diff_count": diff_count}
                        )
            ...
            if "volume" in base_df.columns and "volume" in compare_df.columns:
                ...
                if volume_diff.max() > 0.5:  # 50%差异
                    volume_diffs.append(
                        {"max_diff": volume_diff.max(), "avg_diff": volume_diff.mean()}
                    )
    ...
    async def validate_with_backtrader(self, df: pd.DataFrame, symbol: str) -> Dict[str, Any]:
        """
        使用Backtrader进行数据验证
        ...
        if not HAS_BACKTRADER:
            return {"passed": None, "errors": ["Backtrader未安装"], "warnings": [], "stats": {}}
    ...
    def generate_report(
        self, validations: List[Dict[str, Any]], output_file: Optional[str] = None
    ) -> str:
        """
        生成验证报告
...
async def validate_data(
    df: pd.DataFrame, symbol: str, level: ValidationLevel = ValidationLevel.STANDARD
) -> Dict[str, Any]:
    """
    验证单个数据源
