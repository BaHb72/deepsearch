...
class RealtimeAdapterHealthConfig(BaseModel):
    """实时数据源 adapter 的探活配置"""

    probe_timeout_seconds: float = Field(
        default=5.0,
        gt=0,
        description="探活调用的超时时间（单位秒）",
    )
    failure_threshold: int = Field(
        default=3,
        ge=1,
        description="连续失败多少次记为不可用",
    )
    cool_down_seconds: float = Field(
        default=30.0,
        ge=0,
        description="失败后再次尝试前的冷却时间（单位秒）",
    )


class RealtimeAdapterSpec(BaseModel):
    """供 orchestrator 使用的 adapter 描述"""

    name: str = Field(..., description="adapter 标识")
    driver: str = Field(
        default="amazingdata",
        description="底层实现类型（如 amazingdata/akshare_polling 等）",
    )
    enabled: bool = Field(default=True, description="是否启用该 adapter")
    priority: int = Field(
        default=100,
        ge=0,
        description="越小优先级越高",
    ...
    options: Dict[str, Any] = Field(
        ...
        description="driver 的自定义参数（如 use_proxy/batch_size 等）",
    )
    health: RealtimeAdapterHealthConfig = Field(
        default_factory=RealtimeAdapterHealthConfig,
        description="探活相关参数",
    )
    metadata: Dict[str, Any] = Field(
        default_factory=dict,
        description="用于 UI/Ops 展示的扩展元数据",
    )


class RealtimeAlertPolicy(BaseModel):
    """实时数据源告警配置"""

    enabled: bool = Field(default=True, description="是否发送告警")
    failure_threshold: int = Field(
        default=3,
        ge=1,
        description="窗口内失败次数超过多少触发告警",
    )
    window_seconds: int = Field(
        default=300,
        ge=1,
        description="统计失败次数的时间窗口（单位秒）",
    )
    channels: List[str] = Field(
        default_factory=list,
        description="告警投递渠道/群组 ID 列表",
    )


class RealtimeDataSourceConfig(BaseModel):
    """`data_sources.realtime` 区块"""

    adapters: List[RealtimeAdapterSpec] = Field(
        default_factory=list,
        description="需要编排的 adapter 列表",
    )
    health_check_interval: int = Field(
        default=120,
        ge=30,
        description="统一探活间隔（单位秒）",
    )
    alert_policy: RealtimeAlertPolicy = Field(
        default_factory=RealtimeAlertPolicy,
        description="实时告警策略",
    )
