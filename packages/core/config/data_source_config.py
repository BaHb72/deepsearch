...
class DataTypeConfig:
    ...
    rate_limit: float  # 请求频率（次/秒）
    max_retries: int  # 最大重试次数
    batch_size: Optional[int] = None  # 批处理大小
    priority: int = 5  # 优先级（1-10）


DataTypeConfigMap = Dict[str, DataTypeConfig]
...
class DataSourceConfigManager:
    ...
    def __init__(self, config_path: Optional[str] = None):
        ...
        # 配置变更回调
        self.change_callbacks: List[Callable[[DataSourceConfig], Any]] = []
        ...
        if self.config_path.exists():
            try:
                ...
                if "data_types" in data:
                    raw_data_types = data["data_types"]
                    data_types: DataTypeConfigMap = {}
                    for key, value in raw_data_types.items():
                        if isinstance(value, DataTypeConfig):
    ...
    def _create_default_config(self) -> DataSourceConfig:
        ...
        for key, value in preset.items():
            if key != "data_types":
                ...
                config.data_types = cast(DataTypeConfigMap, value)
                ...
                if hasattr(self.config, key):
                    ...
                    if key == "data_types" and isinstance(value, dict):
                        for dt_key, dt_value in value.items():
                            if isinstance(dt_value, DataTypeConfig):
                                self.config.data_types[dt_key] = dt_value
                                continue
    ...
    def get_data_type_config(self, data_type: str) -> DataTypeConfig:
        ...
        # 返回默认配置
        return DataTypeConfig(cache_ttl=60, request_timeout=10.0, rate_limit=5.0, max_retries=3)

    def register_change_callback(self, callback: Callable[[DataSourceConfig], Any]) -> None:
        """注册配置变更回调"""
    ...
    def _notify_change(self):
        ...
        for callback in self.change_callbacks:
            try:
                result = callback(self.config)
                if inspect.isawaitable(result):
                    asyncio.create_task(cast(Awaitable[Any], result))
            except Exception as exc:
                logger.error(f"配置变更回调失败: {exc}")
    ...
    def get_recommendation(self) -> Recommendation:
        ...
        """
        # 获取当前时间
        from core.utils.time.market_time import MarketTimeUtil

        MarketTimeUtil.get_current_session()
