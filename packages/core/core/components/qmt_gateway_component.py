...
from core.infrastructure.providers.datafeed.qmt.gateway import (
...
from core.infrastructure.providers.datafeed.qmt.models import OrderBook, TickData, TradeData
from core.infrastructure.providers.datafeed.qmt.models.trade import OrderSide
from core.infrastructure.providers.datafeed.qmt.receiver import QMTReceiver
from core.infrastructure.providers.interfaces.payloads import ReceiverStats
...
class QMTGatewayComponent(AsyncComponent):
    ...
    def _initialize_runtime_state(self) -> None:
        ...
        self._cache_ttl: float = float(data_config.get("cache_ttl", 60))
        ...
        self.stats: Dict[str, Any] = {
            ...
            "last_update": None,
            "start_time": None,
            "processing_times": [],  # 记录处理时长
            "error_count": 0,
        }
        self._batch_buffer: List[Any] = []
        self._batch_size = data_config.get("batch_size", 100)
        self._flush_interval = data_config.get("flush_interval", 0.1)
    ...
    async def _handle_level2_batch(self, client_id: str, msg: Dict):
        ...
        try:
            ...
            # 使用列表推导式优化
            from core.infrastructure.providers.datafeed.qmt.models import OrderBookLevel

            orderbook.bid_levels = [
                OrderBookLevel(price=p, volume=v) for p, v in zip(bid_prices, bid_volumes) if p > 0
            ]

            orderbook.ask_levels = [
                OrderBookLevel(price=p, volume=v) for p, v in zip(ask_prices, ask_volumes) if p > 0
            ]
    ...
    async def _flush_batch(self):
        ...
        try:
            ...
            if tick_batch:
                event = Event(
                    type=EVENT_QMT_TICK, data={"batch": [tick.to_dict() for tick in tick_batch]}
                )
                ...
                for tick in tick_batch:
                    self.message_bus.publish(
                        topic=f"QMT.TICK.{tick.symbol}", message=tick.to_dict()
                    )
            ...
            if trade_batch:
                event = Event(
                    type=EVENT_QMT_TRADE, data={"batch": [trade.to_dict() for trade in trade_batch]}
                )
                self.event_engine.put(event)
                ...
                for trade in trade_batch:
                    self.message_bus.publish(
                        topic=f"QMT.TRADE.{trade.symbol}", message=trade.to_dict()
                    )
    ...
    def get_latest_orderbook(self, symbol: str) -> Optional[Dict]:
        ...
        if orderbook:
            logger.info(
                f"从缓存获取盘口数据: {symbol}, 缓存中有 {len(self._orderbook_cache)} 个股票"
            )
            return self._orderbook_to_dict(orderbook)
        else:
            logger.info(
                f"缓存中没有 {symbol} 的盘口数据, 缓存股票: {list(self._orderbook_cache.keys())}"
            )
            return None
