...
class AsyncWrapper:
    ...
    def __init__(self, timeout: float = 10.0):
        ...
        self.executor: Optional[Executor] = None  # 线程池执行器
    ...
    async def call_with_timeout(
        ...
        try:
            ...
            result = await asyncio.wait_for(
                loop.run_in_executor(self.executor, functools.partial(func, *args, **kwargs)),
                timeout=timeout,
            )
            return result
    ...
    async def batch_call(
        self, calls: list, max_concurrent: int = 5, timeout: Optional[float] = None
    ) -> list:
        """
        批量异步调用
        ...
        async def _call_with_semaphore(func, args, kwargs):
            async with semaphore:
                return await self.call_with_timeout(func, *args, timeout=timeout, **kwargs)

        tasks = [
            _call_with_semaphore(func, args or (), kwargs or {}) for func, args, kwargs in calls
        ]
...
def wrap_akshare_func(func: Callable, timeout: float = 10.0):
    ...
    async def async_func(*args, **kwargs):
        return await wrapper.call_with_timeout(func, *args, timeout=timeout, **kwargs)
...
class AkShareAsync:
    ...
    def __getattr__(self, name):
        ...
        try:
            ...
            # 创建异步包装
            async def async_func(*args, **kwargs):
                return await self.wrapper.call_with_timeout(func, *args, **kwargs)
    ...
    async def parallel_fetch(self, fetch_tasks: list, max_concurrent: int = 5):
        """
        并行获取多个数据
        ...
        return await self.wrapper.batch_call(calls, max_concurrent=max_concurrent)
