...
class LogStandard:
    ...
    @staticmethod
    def component_lifecycle(component_name: str, state: str, **kwargs):
        """Log component lifecycle events"""
        return f"Component [{component_name}] {state}"
    
    @staticmethod
    def connection_event(target: str, event: str, **kwargs):
        """Log connection events"""
        return f"Connection [{target}] {event}"
    
    @staticmethod
    def request_event(method: str, url: str, event: str, **kwargs):
        """Log request events"""
        return f"Request [{method} {url}] {event}"
    
    @staticmethod
    def data_event(source: str, event: str, **kwargs):
        """Log data events"""
