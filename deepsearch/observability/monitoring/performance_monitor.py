...
class PerformanceMonitor:
    ...
    def _add_metric(self, metric: PerformanceMetric):
        ...
        # 限制队列大小
        if len(self.metrics[metric.metric_type]) > 10000:
    ...
    def get_statistics(self, duration: Optional[int] = None) -> Dict[str, Any]:
        ...
        # 计算缓存命中率
        cache_hits = sum(
            1 for m in self.metrics[MetricType.CACHE_HIT_RATE]
        ...
        return {
            ...
            'workers': {
                'failures': self.stats['worker_failures']
            },
    ...
    def _check_alerts(self):
        ...
        if stats['summary']['error_rate'] > self.alert_thresholds['error_rate']:
            self._trigger_alert(
                'HIGH_ERROR_RATE',
                f"错误率过高: {stats['summary']['error_rate']:.2%}"
            )
        
        # 检查延迟
        if stats['latency'] and stats['latency'].get('p99', 0) > self.alert_thresholds['latency_p99']:
            self._trigger_alert(
                'HIGH_LATENCY',
                f"P99延迟过高: {stats['latency']['p99']:.2f}秒"
            )
        ...
        if stats['cache']['hit_rate'] < self.alert_thresholds['cache_hit_rate']:
            self._trigger_alert(
                'LOW_CACHE_HIT_RATE',
                f"缓存命中率过低: {stats['cache']['hit_rate']:.2%}"
            )
    ...
    def generate_report(self) -> str:
        ...
        report.append(f"- 成功率: {(self.stats['successful_requests'] / max(self.stats['total_requests'], 1)):.2%}")
        ...
        for label, stats in [("1分钟", stats_1m), ("5分钟", stats_5m), ("1小时", stats_1h)]:
            if stats['latency']:
                lat = stats['latency']
                report.append(
                    f"| {label} | {lat.get('p50', 0):.3f}s | "
                    f"{lat.get('p90', 0):.3f}s | {lat.get('p99', 0):.3f}s | "
                    f"{lat.get('avg', 0):.3f}s |"
                )
