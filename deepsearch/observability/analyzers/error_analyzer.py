...
from deepsearch.observability.logging.monitoring_logger import ErrorType, ErrorInfo
...
class ErrorAnalyzer:
    ...
    def __init__(self):
        ...
        # 修复建议库
        self.fix_suggestions = self._init_fix_suggestions()
    ...
    def analyze_error(self, error_info: ErrorInfo, source_type: str, metadata: Dict) -> Dict:
        """分析单个错误"""
        with self.lock:
            # 更新统计
    ...
    def _cluster_error(self, error_info: ErrorInfo, source_type: str):
        """错误聚类"""
        error_message = error_info.error_message or ""
    ...
    def get_error_patterns_report(self) -> Dict:
        ...
        with self.lock:
            # 统计最常见的错误模式
            top_patterns = sorted(
                self.error_stats["by_pattern"].items(),
                key=lambda x: x[1],
                reverse=True
            )[:10]
                ...
                "top_patterns": [
                    {
                        ...
                        "count": count,
                        "pattern": next(
                            (p for p in self.error_patterns if p.pattern_id == pid),
                            None
                        )
                    }
