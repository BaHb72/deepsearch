...
class ConnectivityTester:
                ...
                try:
                    ...
                    async with session.get(
                        ...
                        if result.is_reachable:
                            ...
                            result.details['edge_location'] = response.headers.get('cf-ray', 'unknown')
                        else:
        ...
        try:
            ...
            try:
                ...
                if test_data is not None and len(test_data) > 0:
                    ...
                    result.details['api_version'] = ak.__version__ if hasattr(ak, '__version__') else 'unknown'
                else:
    ...
    async def _test_database(self) -> ConnectivityTestResult:
        ...
        try:
            ...
            # Redis连接测试
            if hasattr(db_config, 'redis') and db_config.redis:
                redis_host = getattr(db_config.redis, 'host', 'localhost')
                redis_port = getattr(db_config.redis, 'port', 6379)
