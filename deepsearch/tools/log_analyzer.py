...
class LogAnalyzer:
    ...
    def analyze_components(self, hours: int = 24) -> Dict[str, Any]:
        ...
        return {
            ...
            "most_active": sorted(
                component_stats.items(), key=lambda x: x[1]["total_logs"], reverse=True
            )[:10],
            "highest_error_rate": sorted(
                component_stats.items(), key=lambda x: x[1]["error_rate"], reverse=True
            )[:10],
        }

    def _parse_log_file(
        self, file_path: Path, cutoff_time: datetime, levels: Optional[List[str]] = None
    ) -> List[Dict[str, Any]]:
        """解析日志文件"""
        logs = []
    ...
    def _parse_log_line(self, line: str) -> Optional[Dict[str, Any]]:
        """解析单行日志"""
        log_entry: Dict[str, Any] = {"raw": line.strip()}
...
def report(log_dir, hours, output):
    """生成综合分析报告"""
    analyzer = LogAnalyzer(log_dir)
    analyzer.generate_report(output, hours)
