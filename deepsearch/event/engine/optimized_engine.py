...
def _load_xxhash() -> Optional[ModuleType]:
    try:
        return import_module("xxhash")
    except ImportError:
        return None


_xxhash_module = _load_xxhash()


def _fallback_hash_factory() -> Any:
    return hashlib.md5()
        ...
        def cleanup():
            while self.running:
                try:
                    ...
                    with self.lock:
                        # 清理过期事件
                        self.seen_events = {k: v for k, v in self.seen_events.items() if v > cutoff}
...
class BatchProcessor:
    ...
    def __init__(self, batch_size: int = 100, batch_timeout: float = 0.1):
        ...
        self.batches: DefaultDict[str, List[Tuple[Event, Callable[..., Any]]]] = defaultdict(list)
    ...
    def add(self, event: Event, callback: Callable):
        ...
        with self.lock:
            ...
            if len(self.batches[batch_key]) >= self.batch_size:
                ...
                # 启动定时器
                timer = threading.Timer(self.batch_timeout, lambda: self._process_batch(batch_key))
                timer.start()
...
class EventMetrics:
    ...
    def __init__(self):
        ...
        self.queue_sizes: Deque[Tuple[datetime, int]] = deque(maxlen=1000)
        self.lock = threading.Lock()
...
class OptimizedEventEngine:
    ...
    def __init__(
        ...
        # 事件队列（优先级队列）
        self.event_queue: queue.PriorityQueue[Event] = queue.PriorityQueue(maxsize=queue_size)

        # 处理器注册表
        self.handlers: DefaultDict[str, List[Dict[str, Any]]] = defaultdict(list)
        ...
        # 运行状态
        self.running = False
        self.worker_threads: List[threading.Thread] = []  # 支持多个工作线程
        self.num_workers = min(4, max_workers // 4)  # 工作线程数量

        # 处理器缓存（优化查找性能）
        self.handler_cache: Dict[str, List[Dict[str, Any]]] = {}
    ...
    def register_handler(
        self, event_type: str, handler: Callable, priority: int = 0, batch: bool = False
    ):
        """
        注册事件处理器
    ...
    def start(self):
        ...
        logger.info(
            f"Optimized event engine started with {self.num_workers} workers and {self.max_workers} thread pool size"
        )
    ...
    def _process_event(self, event: Event):
        ...
        for handler_info in handlers:
            if handler_info["batch"] and event.batch_enabled:
                ...
                # 立即处理
                self.executor.submit(self._execute_handler, handler_info, event)
...
def benchmark_event_engine():
    ...
    for i in range(event_count):
        event = Event(type=f"TEST_{i % 10}", data={"value": i}, priority=random.randint(0, 5))
        engine.emit(event)
    ...
    return {"duration": duration, "throughput": throughput, "metrics": metrics}
