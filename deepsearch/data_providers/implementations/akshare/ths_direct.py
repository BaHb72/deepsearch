...
class ThsDirectProvider:
    ...
    async def get_concept_index(self, symbol: str, start_date: str = "20230101", end_date: str = "20250131") -> Dict[str, Any]:
        """获取概念板块指数数据"""
        try:
    ...
    async def get_concept_constituents(self, symbol: str) -> Dict[str, Any]:
        ...
        """
        try:
            logger.info(f"获取概念板块汇总信息（成份股功能暂不可用）")
            df = await asyncio.get_event_loop().run_in_executor(
