...
class KlineCache:
    ...
    def __init__(self, redis_client=None, use_duckdb: bool = True):
        """
        初始化缓存管理器
        
        Args:
            redis_client: Redis客户端
            use_duckdb: 是否使用DuckDB（否则使用PostgreSQL）
        """
        # L1: 内存缓存
        self.memory_cache = {}
        self.memory_ttl = {
    ...
    def _init_database(self):
        ...
        if self.use_duckdb:
            ...
            try:
                import duckdb
                self.db_path = config.database.analytics.path
                Path(self.db_path).parent.mkdir(parents=True, exist_ok=True)
                conn = duckdb.connect(self.db_path)
                cursor = conn.cursor()
            except ImportError:
                logger.warning("DuckDB未安装，降级使用SQLite")
                self.use_duckdb = False
                self.db_path = "./data/kline_cache.db"
                Path(self.db_path).parent.mkdir(parents=True, exist_ok=True)
                conn = sqlite3.connect(self.db_path)
                cursor = conn.cursor()
        else:
            # 使用PostgreSQL（暂时使用SQLite作为后备）
            # TODO: 集成PostgreSQL连接池
            self.db_path = "./data/kline_cache.db"
            Path(self.db_path).parent.mkdir(parents=True, exist_ok=True)
            conn = sqlite3.connect(self.db_path)
            cursor = conn.cursor()
        ...
        db_type = "DuckDB" if self.use_duckdb else "SQLite"
        logger.info(f"K线缓存数据库初始化完成 ({db_type}): {self.db_path}")
