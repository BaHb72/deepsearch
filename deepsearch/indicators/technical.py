...
class TechnicalIndicators:
    ...
    def vwap(self, df: pd.DataFrame, session_reset: bool = True) -> NumericSeries:
        ...
        # 计算典型价格
        typical_price = cast(NumericSeries, (df["high"] + df["low"] + df["close"]) / 3)
        volume_series = cast(NumericSeries, df["volume"].astype(float, copy=False))
        ...
        if session_reset and "session_start" in df.columns:
            ...
            cumulative_volume_series = cast(NumericSeries, volume_series.cumsum())
    ...
    def bias(self, df: pd.DataFrame, period: int = 6, price_col: str = "close") -> NumericSeries:
        ...
        """
        prices = self._prepare_data(df, price_col)
        price_series = cast(NumericSeries, pd.Series(prices, index=df.index, name=price_col))
        ma = cast(NumericSeries, price_series.rolling(period).mean())
    ...
    def volume_ratio(self, df: pd.DataFrame, period: int = 5) -> NumericSeries:
        ...
        # 计算过去N日平均成交量
        volume_series = cast(NumericSeries, df["volume"].astype(float, copy=False))
        avg_volume = cast(NumericSeries, volume_series.rolling(period).mean().shift(1))
        safe_avg_volume = cast(NumericSeries, avg_volume.replace(0.0, np.nan))
