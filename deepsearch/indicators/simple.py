...
class SimpleIndicators:
    ...
    def _ensure_numeric(self, df: pd.DataFrame, column: str) -> NumericSeries:
        """确保列为浮点数序列，便于类型检查"""
        series = df[column].astype(float, copy=False)
    ...
    def sma(self, df: pd.DataFrame, period: int = 20, price_col: str = "close") -> NumericSeries:
        ...
        return cast(NumericSeries, price_series.rolling(window=period, min_periods=1).mean())

    def ema(self, df: pd.DataFrame, period: int = 20, price_col: str = "close") -> NumericSeries:
        """指数移动平均线 (EMA)"""
        price_series = self._ensure_numeric(df, price_col)
        return cast(NumericSeries, price_series.ewm(span=period, adjust=False).mean())
    ...
    def wma(self, df: pd.DataFrame, period: int = 20, price_col: str = "close") -> NumericSeries:
        ...
        return cast(
            NumericSeries,
            price_series.rolling(period, min_periods=1).apply(weighted_average, raw=True),
        )
    ...
    def macd(
        ...
        ema_fast_filled = cast(NumericSeries, ema_fast.fillna(0.0))
        ema_slow_filled = cast(NumericSeries, ema_slow.fillna(0.0))
        macd_line = cast(NumericSeries, ema_fast_filled - ema_slow_filled)
        signal_line = cast(
            NumericSeries,
            macd_line.ewm(span=signal_period, adjust=False).mean().fillna(0.0),
        )
    ...
    def rsi(self, df: pd.DataFrame, period: int = 14, price_col: str = "close") -> NumericSeries:
        """相对强弱指数 (RSI)"""
        price_series = self._ensure_numeric(df, price_col)
        delta = cast(NumericSeries, price_series.diff())
        gain = cast(
            NumericSeries,
            delta.clip(lower=0.0).rolling(window=period, min_periods=period).mean(),
        )
        loss = cast(
            NumericSeries,
    ...
    def stoch(self, df: pd.DataFrame, k_period: int = 14, d_period: int = 3) -> pd.DataFrame:
        """随机指标 (Stochastic)"""
        low_series = self._ensure_numeric(df, "low")
        high_series = self._ensure_numeric(df, "high")
        close_series = self._ensure_numeric(df, "close")

        lowest_low = cast(
            NumericSeries, low_series.rolling(window=k_period, min_periods=k_period).min()
        )
        highest_high = cast(
            NumericSeries, high_series.rolling(window=k_period, min_periods=k_period).max()
        )

        highest_high_filled = cast(NumericSeries, highest_high.fillna(0.0))
        lowest_low_filled = cast(NumericSeries, lowest_low.fillna(0.0))
        ...
        d_percent = cast(
            NumericSeries, k_percent.rolling(window=d_period, min_periods=d_period).mean()
        ).fillna(0.0)
    ...
    def bollinger_bands(
        self, df: pd.DataFrame, period: int = 20, std_dev: float = 2.0, price_col: str = "close"
    ) -> pd.DataFrame:
        """布林带 (Bollinger Bands)"""
        price_series = self._ensure_numeric(df, price_col)
    ...
    def atr(self, df: pd.DataFrame, period: int = 14) -> NumericSeries:
        """平均真实波幅 (ATR)"""
        high = self._ensure_numeric(df, "high")
        low = self._ensure_numeric(df, "low")
        close = self._ensure_numeric(df, "close")
        prev_close = cast(NumericSeries, close.shift())
        ...
        true_range = cast(NumericSeries, ranges.max(axis=1, skipna=True))

        return cast(NumericSeries, true_range.rolling(window=period, min_periods=period).mean())
    ...
    def vwap(self, df: pd.DataFrame) -> NumericSeries:
        """成交量加权平均价格 (VWAP)"""
