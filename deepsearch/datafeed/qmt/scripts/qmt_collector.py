...
def download_history_data(stock_code, period='1d', start_time='', end_time='', dividend_type='front'):
    ...
    while g_running:
        ...
        try:
            ...
            for symbol in g_subscribed[:]:  # Copy list to avoid modification conflicts
                ...
                if orderbook_data:
                    orderbook_batch.append({
                        'type': 'LEVEL2',
                        'data': orderbook_data
                    })
                    # Log every push in debug mode or every 10th push normally
                    if DEBUG_MODE or push_count % 10 == 0:
                        print('[Push] Orderbook for %s: bid=%d, ask=%d' %
                              (symbol, len(orderbook_data.get('bid_price', [])),
                               len(orderbook_data.get('ask_price', []))))

                # Send immediately if we have data (don't wait for batch in debug mode)
                should_send = (DEBUG_MODE and (tick_batch or orderbook_batch)) or \
                              (len(tick_batch) + len(orderbook_batch)) >= BATCH_SIZE or \
                              (time.time() - last_push) > 1.0
                ...
                if should_send and (tick_batch or orderbook_batch):
                    ...
                    print('[Push] Sent %d tick + %d orderbook updates (push #%d)' %
    ...
    try:
        while g_running:
            ...
            if int(time.time()) % 60 == 0:
                ...
                print('[Stats] Uptime: %ds, Sent: %d, Errors: %d' %
