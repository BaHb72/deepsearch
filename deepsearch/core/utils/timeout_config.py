...
class TimeoutConfig:
    ...
    def get_timeout(self, attempt: int = 0) -> float:
        ...
        # 指数退避，但不超过最大值
        timeout = min(
            self.default * (self.retry_multiplier ** attempt),
            self.max
        )
        return timeout
...
class TimeoutManager:
    ...
    DEFAULT_CONFIGS: Dict[TimeoutCategory, TimeoutConfig] = {
        ...
        TimeoutCategory.NETWORK_REALTIME: TimeoutConfig(
            default=10.0, min=5.0, max=30.0
        ),
        TimeoutCategory.NETWORK_HISTORICAL: TimeoutConfig(
            default=30.0, min=10.0, max=120.0
        ),
        TimeoutCategory.NETWORK_BATCH: TimeoutConfig(
            default=60.0, min=30.0, max=300.0
        ),
        TimeoutCategory.NETWORK_HEALTH: TimeoutConfig(
            default=5.0, min=2.0, max=10.0
        ),

        # 数据库操作 - 相对较短的超时
        TimeoutCategory.DB_CONNECT: TimeoutConfig(
            default=10.0, min=5.0, max=30.0
        ),
        TimeoutCategory.DB_QUERY: TimeoutConfig(
            default=30.0, min=5.0, max=120.0
        ),
        TimeoutCategory.DB_TRANSACTION: TimeoutConfig(
            default=60.0, min=10.0, max=300.0
        ),
        TimeoutCategory.DB_HEALTH: TimeoutConfig(
            default=5.0, min=2.0, max=10.0
        ),

        # 缓存操作 - 应该很快
        TimeoutCategory.CACHE_GET: TimeoutConfig(
            default=5.0, min=1.0, max=10.0
        ),
        TimeoutCategory.CACHE_SET: TimeoutConfig(
            default=5.0, min=1.0, max=10.0
        ),
        TimeoutCategory.CACHE_DELETE: TimeoutConfig(
            default=5.0, min=1.0, max=10.0
        ),
        TimeoutCategory.CACHE_FLUSH: TimeoutConfig(
            default=10.0, min=5.0, max=30.0
        ),

        # 消息总线 - 取决于消息处理复杂度
        TimeoutCategory.MSG_PUBLISH: TimeoutConfig(
            default=5.0, min=1.0, max=30.0
        ),
        TimeoutCategory.MSG_SUBSCRIBE: TimeoutConfig(
            default=5.0, min=1.0, max=30.0
        ),
        TimeoutCategory.MSG_CONSUME: TimeoutConfig(
            default=30.0, min=5.0, max=120.0
        ),

        # 组件生命周期 - 给予足够时间
        TimeoutCategory.COMPONENT_INIT: TimeoutConfig(
            default=30.0, min=10.0, max=120.0
        ),
        TimeoutCategory.COMPONENT_START: TimeoutConfig(
            default=30.0, min=10.0, max=120.0
        ),
        TimeoutCategory.COMPONENT_STOP: TimeoutConfig(
            default=30.0, min=10.0, max=120.0
        ),
        TimeoutCategory.COMPONENT_HEALTH: TimeoutConfig(
            default=5.0, min=2.0, max=10.0
        ),
    }
