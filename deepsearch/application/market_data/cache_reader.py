...
class MarketDataCacheReader:
    ...
    async def fetch_order_imbalance(
        ...
        envelope = await self._get_envelope(f"market:order-imbalance:{window}")
        if not envelope:
    ...
    async def fetch_auction_quality(
        ...
        for board in boards:
            if not board:
                continue
            envelope = await self._get_envelope(f"market:auction:{board}")
            if envelope:
                envelopes.append(envelope)
    ...
    async def fetch_board_universe(
        ...
        envelope = await self._get_envelope("market:boards")
        if not envelope:
            return {}, None
