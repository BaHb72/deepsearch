...
用法:
    from deepsearch.application.data.handlers import KlineDataHandler
...
from deepsearch.ports.data.semantic_types import AssetSpec, Timeframe
...
class KlineDataHandler:
    ...
    def close(self) -> np.ndarray:
        """返回收盘价 1D 数组。"""
        return self.to_numpy(["close"]).flatten()
    
    @property
    def volume(self) -> np.ndarray:
        """返回成交量 1D 数组。"""
        return self.to_numpy(["volume"]).flatten()
    ...
    def from_dataframe(
        ...
        for timestamp, row in df.iterrows():
            bar = KlineBar(
                ...
                turnover=Decimal(str(row["turnover"])) if "turnover" in row and pd.notna(row["turnover"]) else None,
            )
            bars.append(bar)
