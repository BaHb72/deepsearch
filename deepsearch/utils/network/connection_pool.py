...
class Connection:
    ...
    async def validate(self) -> bool:
        ...
        if self.pool.validator:
            try:
                return await self.pool.validator(self.conn)
    ...
    async def close(self):
        ...
        if not self.closed:
            ...
            if self.pool.closer:
                try:
                    await self.pool.closer(self.conn)
                except Exception as e:
...
class ConnectionPool:
    ...
    def __init__(
        ...
        self.pool: asyncio.Queue = asyncio.Queue(maxsize=self.config.max_size)
