...
class DataValidationStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        if self.params.check_ohlc:
            ...
            # Low应该是最低价
            if l > min(o, c):
                self.data_errors.append(f"{current_date}: Low({l}) > Min(Open({o}), Close({c}))")
                self.stats['ohlc_errors'] += 1
                is_valid = False
            
            # High >= Low
            if h < l:
                self.data_errors.append(f"{current_date}: High({h}) < Low({l})")
                self.stats['ohlc_errors'] += 1
                is_valid = False
        ...
        # 更新价格范围
        self.stats['min_price'] = min(self.stats['min_price'], l)
...
class ComparisonStrategy(bt.Strategy):
    ...
    def stop(self):
        ...
        # 计算平均差异
        if self.price_diffs:
            self.stats['avg_price_diff'] = sum(d['avg_diff'] for d in self.price_diffs) / len(self.price_diffs)
        
        if self.volume_diffs:
            self.stats['avg_volume_diff'] = sum(d['diff'] for d in self.volume_diffs) / len(self.volume_diffs)
        ...
        if self.params.printlog:
            ...
            if self.price_diffs:
                ...
                for diff in sorted_diffs[:5]:
                    self.log(f'  {diff["date"]}: 数据源{diff["source"]} 差异 {diff["max_diff"]:.2%}')
        ...
        if self.params.printlog:
            ...
            if self.indicator_values:
                ...
                for values in self.indicator_values[-3:]:
                    self.log(f"  {values['date']}: RSI={values['rsi']:.2f if values['rsi'] else 0}, "
