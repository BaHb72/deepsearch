...
logging.getLogger('matplotlib.fontmanager').setLevel(logging.WARNING)
...
# 抑制Backtrader的调试日志
logging.getLogger('backtrader').setLevel(logging.INFO)
logging.getLogger('backtrader.plot').setLevel(logging.WARNING)
...
class BacktestEngine:
    ...
    async def add_data(
                        ...
                        for param_name in ['long_period', 'period', 'ma_period', 'slow_period']:
                            if hasattr(params, param_name):
                                ...
                                if isinstance(period_value, (int, float)):
                                    max_lookback = max(max_lookback, int(period_value) + 10)  # 额外增加10天缓冲
        ...
        logger.info(f"自动扩展数据获取范围: 原始开始日期 {start_date.strftime('%Y-%m-%d')}, "
        ...
        for symbol in symbols:
            ...
            if not df.empty:
                ...
                if len(df) < max_lookback:
                    logger.warning(f"数据量不足: {symbol} 只有 {len(df)} 条记录，"
                ...
                logger.error(f"1. 股票代码是否正确")
                logger.error(f"2. 日期范围是否有效")
                logger.error(f"3. 数据源是否可用")
    ...
    def plot_results(self, save_path: Optional[str] = None, use_backtrader_plot: bool = True) -> Optional[str]:
        """
        绘制回测结果图表
        ...
        try:
            ...
            if not use_backtrader_plot or figs is None:
                ...
                # 2. 收益率
                axes[1].plot(equity_df['date'], equity_df['cumulative_returns'] * 100, 
                ...
                # 3. 回撤
                axes[2].fill_between(equity_df['date'], equity_df['drawdown'] * 100, 0, 
                                    color='red', alpha=0.3)
                axes[2].set_title('回撤')
        ...
        for values in itertools.product(*param_values):
            ...
            try:
                ...
                loop.run_until_complete(
                    self.add_data(symbols, start_date, end_date)
                )
                loop.close()
    ...
    def export_results(self, format: str = 'json') -> Union[str, pd.DataFrame]:
        ...
        if format == 'json':
            ...
            with pd.ExcelWriter('backtest_results.xlsx', engine='openpyxl') as writer:
                ...
                # 交易记录
                pd.DataFrame(data['trades']).to_excel(
                    writer, sheet_name='Trades', index=False
                )
                # 权益曲线
                pd.DataFrame(data['equity_curve']).to_excel(
