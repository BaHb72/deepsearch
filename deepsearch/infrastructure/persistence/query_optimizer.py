...
from typing import DefaultDict, Deque, TypedDict, Union
...
class JoinInfo(TypedDict, total=False):
    """JOIN 子句信息。"""
...
class NPlusOneDetection(TypedDict):
    """N+1 查询检测结果。"""
...
class IndexSuggestion(TypedDict, total=False):
    """索引建议。"""
...
class SlowQueryEntry(TypedDict):
    """慢查询原始记录。"""
...
class QueryStatisticsSnapshot(TypedDict):
    """��ѯͳ������ӳ��ṹ"""
...
class OptimizationReport(TypedDict):
    ...
    n_plus_one_detections: list[NPlusOneDetection]
...
class QueryOptimizer:
    ...
    def __init__(self):
        if not hasattr(self, "_initialized"):
            self.query_stats: DefaultDict[str, QueryStats] = defaultdict(lambda: QueryStats(""))
            self.slow_queries: Deque[SlowQueryEntry] = deque(maxlen=100)
    ...
    def analyze_query(self, query: Union[str, Query]) -> QueryAnalysis:
        """分析查询"""
        query_str = str(query)
        ...
        self.query_cache[query_str] = analysis
    ...
    def _check_index_usage(self, query: str, tables: Sequence[str]) -> bool:
        """检查是否使用索引"""
    ...
    def detect_n_plus_one(self, session: Session) -> list[NPlusOneDetection]:
        """检测N+1查询问题"""
    ...
    def suggest_indexes(self, engine: Engine) -> list[IndexSuggestion]:
        """建议索引"""
    ...
    def _suggest_composite_indexes(self) -> list[IndexSuggestion]:
        """建议复合索引"""
        suggestions: list[IndexSuggestion] = []

        # 分析经常一起使用的字段组合
        field_combinations: DefaultDict[tuple[str, ...], int] = defaultdict(int)
    ...
    def record_query_execution(self, query: str, duration: float, error: bool = False):
        ...
        if duration > self.slow_threshold:
            self.slow_queries.append(
                {"query": query, "duration": duration, "timestamp": datetime.now(), "error": error}
            )
    ...
    def get_slow_queries(self, top_n: int = 10) -> list[SlowQuerySummary]:
        """获取慢查询"""
        # 按执行时间排序
        sorted_queries = sorted(self.query_stats.items(), key=lambda x: x[1].avg_time, reverse=True)
    ...
    def _get_statistics(self) -> QueryStatisticsSnapshot:
        """��ȡͳ����Ϣ"""
    ...
    def handle_error(exception_context):
        if "query_start_time" in exception_context.connection.info:
            ...
            query_optimizer.record_query_execution(
                exception_context.statement, total_time, error=True
            )
