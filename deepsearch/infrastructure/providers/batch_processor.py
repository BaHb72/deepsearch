...
class BatchProcessor:
        ...
        async with self.lock:
            ...
            if len(self.pending_batches[api_name]) >= batch_size:
                ...
                task = asyncio.create_task(
                    self._delayed_process(api_name, executor)
                )
    ...
    async def _process_parallel_batch(
        ...
        tasks = []
        for request in batch:
            task = executor(api_name, request.params)
        ...
        # 设置结果
        for (request, _), result in zip(tasks, results):
    ...
    def get_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            ...
            'avg_batch_size': self.stats['batched_requests'] / max(self.stats['batch_executions'], 1),
