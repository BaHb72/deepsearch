# encoding:utf-8
"""
AmazingData 外部 Worker 模块。

通过 multiprocessing.Connection 监听来自宿主进程的请求，
复用 AmazingDataProcessProxy 既有的 worker 循环逻辑。
"""
...
def _configure_logging(level: str) -> None:
    ...
    AmazingDataProcessProxy._ensure_worker_file_logger(level.upper())
...
def main() -> None:
    parser = argparse.ArgumentParser(description="AmazingData external worker")
    ...
    except Exception as exc:  # pragma: no cover - 异常时便于排查
        logger.opt(exception=exc).error("Worker crashed with unhandled exception")
    finally:
        try:
            conn.close()
        except Exception:
