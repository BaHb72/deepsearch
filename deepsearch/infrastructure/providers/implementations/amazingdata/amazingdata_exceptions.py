...
class ErrorHandler:
    ...
    def handle_login_error(error: Exception) -> AmazingDataException:
        ...
        if "timeout" in error_msg:
            return AmazingDataConnectionError(
                "登录超时", AmazingDataErrorCode.NETWORK_TIMEOUT, original_exception=error
            )
            ...
            return AmazingDataConnectionError(
                "网络连接失败", AmazingDataErrorCode.CONNECTION_FAILED, original_exception=error
            )
        else:
            return AmazingDataConnectionError(
                f"登录失败: {error}", AmazingDataErrorCode.LOGIN_FAILED, original_exception=error
            )

    @staticmethod
    def handle_query_error(error: Exception, query_type: str | None = None) -> AmazingDataException:
        """处理查询错误"""
        error_msg = str(error).lower()
        ...
        if "no data" in error_msg or "empty" in error_msg:
            ...
            return AmazingDataQueryError(
                "无效的股票代码", AmazingDataErrorCode.INVALID_SYMBOL, original_exception=error
            )
        elif "invalid period" in error_msg:
            return AmazingDataQueryError(
                "无效的周期参数", AmazingDataErrorCode.INVALID_PERIOD, original_exception=error
            )
        elif "rate limit" in error_msg:
            return AmazingDataRateLimitError(
                "查询频率超限", retry_after=60, original_exception=error  # 默认60秒后重试
            )
        else:
            return AmazingDataQueryError(
                f"查询失败: {error}",
    ...
    def handle_subscription_error(error: Exception, symbols: list[str] | None = None) -> AmazingDataException:
        ...
        if "limit" in error_msg or "exceed" in error_msg:
            ...
            return AmazingDataSubscriptionError(
                "订阅不存在", AmazingDataErrorCode.SUBSCRIPTION_NOT_FOUND, original_exception=error
            )
