    ...
    while g_running:
        ...
        try:
            ...
            for symbol in g_subscribed[:]:  # 复制列表，避免修改时冲突
                ...
                # 批量发送
                if (len(tick_batch) + len(orderbook_batch)) >= BATCH_SIZE or (time.time() - last_push) > 1.0:
                    # 发送Tick数据
                    ...
                    if tick_batch or orderbook_batch:
                        print('[Push] Sent %d tick + %d orderbook updates' % (len(tick_batch), len(orderbook_batch)))
