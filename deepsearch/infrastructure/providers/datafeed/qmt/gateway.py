...
class QMTGateway(Component):
    ...
    def get_status(self) -> Dict:
        ...
        orderbook_count = cast(int, self.stats.get("orderbook_count", 0))
        trade_count = cast(int, self.stats.get("trade_count", 0))
    ...
    async def _handle_tick_data(self, client_id: str, msg: MessageEnvelope) -> None:
        ...
        try:
            ...
            # 更新统计
            tick_count_value = self.stats.get("tick_count", 0)
            tick_count = tick_count_value if isinstance(tick_count_value, int) else 0
            self.stats["tick_count"] = tick_count + 1
            self.stats["last_update"] = time.time()

        except Exception as e:
            logger.error(f"处理Tick数据失败: {e}")

    async def _handle_level2_data(self, client_id: str, msg: MessageEnvelope) -> None:
        """处理Level2十档盘口数据"""
        try:
            data = cast(Dict[str, Any], msg.get("data", {}))
            if not isinstance(data, dict):
            ...
            logger.info(
                f"[LEVEL2] 收到盘口数据: symbol={symbol}, client={client_id}, data_keys={list(data.keys())}"
            )

            # 创建OrderBook对象
            orderbook = OrderBook(symbol=symbol, timestamp=data.get("timestamp", time.time()))
            ...
            for i in range(min(len(bid_prices), len(bid_volumes))):
                if bid_prices[i] > 0:
                    ...
                    orderbook.bid_levels.append(
                        OrderBookLevel(price=bid_prices[i], volume=bid_volumes[i])
                    )
            ...
            for i in range(min(len(ask_prices), len(ask_volumes))):
                if ask_prices[i] > 0:
                    ...
                    orderbook.ask_levels.append(
                        OrderBookLevel(price=ask_prices[i], volume=ask_volumes[i])
                    )
            ...
            # 更新统计
            orderbook_value = self.stats.get("orderbook_count", 0)
            orderbook_count = orderbook_value if isinstance(orderbook_value, int) else 0
    ...
    async def _handle_trade_data(self, client_id: str, msg: MessageEnvelope) -> None:
        """处理逐笔成交数据"""
        try:
            data = cast(Dict[str, Any], msg.get("data", {}))
            if not isinstance(data, dict):
            ...
            # 更新统计
            trade_value = self.stats.get("trade_count", 0)
            trade_count = trade_value if isinstance(trade_value, int) else 0
    ...
    async def _handle_batch_data(self, client_id: str, msg: MessageEnvelope) -> None:
        """处理批量数据"""
        ...
        for item in batch_data:
            if isinstance(item, dict):
                ...
                envelope = cast(MessageEnvelope, item)
                if msg_type == "TICK":
                    await self._handle_tick_data(client_id, envelope)
                elif msg_type == "LEVEL2":
                    await self._handle_level2_data(client_id, envelope)
                elif msg_type == "TRADE":
                    await self._handle_trade_data(client_id, envelope)

    def _publish_tick_event(self, tick: TickData):
        """发布Tick事件"""
        event = Event(type=EVENT_QMT_TICK, data=tick.to_dict())
        self.event_engine.put(event)

        # 同时发布到消息总线
        self.message_bus.publish(topic=f"QMT.TICK.{tick.symbol}", message=tick.to_dict())
    ...
    def _publish_orderbook_event(self, orderbook: OrderBook):
        ...
        event = Event(
            ...
            data={
                ...
                "bid_levels": [
                    {"price": level.price, "volume": level.volume} for level in orderbook.bid_levels
                ],
                "ask_levels": [
                    {"price": level.price, "volume": level.volume} for level in orderbook.ask_levels
                ],
            },
        )
        self.event_engine.put(event)
        ...
        # 同时发布到消息总线
        self.message_bus.publish(topic=f"QMT.ORDERBOOK.{orderbook.symbol}", message=event.data)

    def _publish_trade_event(self, trade: TradeData):
        """发布成交事件"""
        event = Event(type=EVENT_QMT_TRADE, data=trade.to_dict())
        self.event_engine.put(event)

        # 同时发布到消息总线
        self.message_bus.publish(topic=f"QMT.TRADE.{trade.symbol}", message=trade.to_dict())

    def _publish_connection_event(self, connected: bool):
        """发布连接状态事件"""
        event = Event(
            type=EVENT_QMT_CONNECTION, data={"connected": connected, "timestamp": time.time()}
        )
        self.event_engine.put(event)
    ...
    def subscribe(self, symbols: List[str]):
        ...
        if self.receiver and hasattr(self.receiver, "client_writers"):
            ...
            logger.warning("无法发送订阅请求，接收器未就绪或无客户端连接")
    ...
    def unsubscribe(self, symbols: List[str]):
        ...
        if self.receiver and hasattr(self.receiver, "client_writers"):
            request_id = f"req_{uuid.uuid4().hex[:8]}"
            unsubscribe_msg = {"type": "UNSUBSCRIBE", "symbols": symbols, "request_id": request_id}
