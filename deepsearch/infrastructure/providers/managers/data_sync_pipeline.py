...
class DataSyncPipeline:
    ...
    async def sync(
        ...
        **fetch_kwargs,
        ...
        sorted_sources = sorted(
            [self._sources[s] for s in sources if s in self._sources],
        ...
        if parallel:
            ...
            # 顺序同步
            for source in sorted_sources:
                result = await self._sync_source(source, table, force_full, **fetch_kwargs)
    ...
    async def _sync_source(
        ...
        **fetch_kwargs,
        ...
        try:
            # 1. Fetch + Normalize
            df, _ = await self._fetch_and_normalize(
                source, table, force_full, **fetch_kwargs
            )
