...
class DataSyncService:
    ...
    async def sync_kline_history(
        ...
        try:
            ...
            logger.info("同步了 %s 条K线数据", len(df))
            ...
            if "Duplicate key" in str(import_error):
                ...
                try:
                    ...
                    logger.info("使用 upsert 方式同步了 %s 条K线数据", len(df))
    ...
    async def sync_stock_info(self):
        ...
        if missing:
            logger.error("股票信息缺少必要字段: %s", ", ".join(sorted(missing)))
            return
        ...
        try:
            ...
            logger.info("同步了 %s 条股票信息", len(stock_info))
        except Exception as exc:
