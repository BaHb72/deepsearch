...
class TurtleTradingStrategy(bt.Strategy):
    ...
    def __init__(self):
        ...
        # 唐奇安通道
        self.donchian_high = bt.indicators.Highest(self.datahigh, period=entry_period)
        self.donchian_low = bt.indicators.Lowest(self.datalow, period=exit_period)
    ...
    def notify_trade(self, trade):
        ...
        if trade.isclosed:
            self.trades.append(
                {"pnl": trade.pnl, "pnlcomm": trade.pnlcomm, "value": self.broker.getvalue()}
            )

            if self.params.printlog:
                self.log(f"交易关闭: 毛利润={trade.pnl:.2f}, " f"净利润={trade.pnlcomm:.2f}")
        ...
        if not self.position:
            ...
            if self.datahigh[0] > self.donchian_high[-1]:
                ...
                if self.dataclose[0] > self.sma[0]:
                    if self.params.printlog:
                        self.log(
                            f"突破买入信号: 价格{self.datahigh[0]:.2f} > "
            ...
            if self.dataclose[0] < self.stop_price:
                if self.params.printlog:
                    self.log(f"止损: 价格{self.dataclose[0]:.2f} < " f"止损价{self.stop_price:.2f}")
    ...
    def stop(self):
        ...
        if self.params.printlog:
            self.log(
                f"海龟策略结束 - 系统{self.params.use_system}, "
