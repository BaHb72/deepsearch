...
class MeanReversionStrategy(bt.Strategy):
    ...
    def notify_trade(self, trade):
        ...
        if trade.isclosed:
            self.trades_count += 1
            if self.params.printlog:
                self.log(f'交易关闭 #{self.trades_count}: '
        ...
        if not self.position:
            ...
            if price <= bb_lower * 1.01 and rsi_value < self.params.rsi_oversold:
                ...
                if self.datas[0].volume[0] > self.volume_sma[0] * 1.2:
                    if self.params.printlog:
                        self.log(f'买入信号: 价格{price:.2f}接近下轨{bb_lower:.2f}, '
                                f'RSI={rsi_value:.2f}')
            ...
            # 注意：A股不支持做空，这里仅作示例
                ...
                if self.params.printlog:
                    self.log(f'卖空信号: 价格{price:.2f}接近上轨{bb_upper:.2f}, '
                            f'RSI={rsi_value:.2f}')
            ...
            if self.entry_type == 'long':
                ...
                # 止盈：价格回归到中线附近
                    ...
                    if profit_pct > 0.01:  # 至少1%利润
                        if self.params.printlog:
                            self.log(f'均值回归止盈: 价格{price:.2f}接近中线{bb_middle:.2f}, '
    ...
    def stop(self):
        ...
        if self.params.printlog:
            ...
            self.log(f'均值回归策略结束 - BB周期: {self.params.bb_period}, '
