...
class MomentumStrategy(bt.Strategy):
    ...
    def next(self):
        ...
        if not self.position:
            ...
            is_obv_rising = self.obv[0] > self.obv_sma[0]
            
            # 综合判断
            if (is_breakout and is_strong_momentum and is_volume_surge and 
                is_trend_up and is_macd_positive):
                
                if self.params.printlog:
                    self.log(f'动量买入信号: 价格{price:.2f}, '
    ...
    def stop(self):
        """策略结束"""
        if self.params.printlog:
            self.log(f'动量策略结束 - 动量周期: {self.params.momentum_period}, '
