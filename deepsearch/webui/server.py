    ...
    async def startup_handler() -> None:
        ...
        try:
            ...
            try:
                ...
                # 使用 DatabaseComponentFactory 直接创建数据库组件
                db_component = DatabaseComponentFactory.create()
                await db_component.initialize_async()
                db_service = DatabaseService(db_component)
                repository = ModuleSourceRepository(db_service)
                get_module_registry().set_repository(repository)
                logger.info("模块数据源配置 Repository 已初始化")
            except Exception as e:
...
def create_app() -> FastAPI:
    ...
    # 模块数据源配置API
    try:
        from deepsearch.webui.api.endpoints.datasources.module_sources_api import (
