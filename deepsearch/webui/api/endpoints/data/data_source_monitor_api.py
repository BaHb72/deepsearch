...
async def get_recommendation(
    access_type: str,
    require_realtime: bool = Query(False, description="是否需要实时数据")
):
    """
    获取推荐的数据源
...
async def get_realtime_monitoring():
    ...
    try:
        ...
        for source_type, metrics in monitor.source_metrics.items():
            realtime_data["source_metrics"][source_type.value] = {
...
async def test_data_access(
    ...
    try:
        ...
        start_time = time.time()
        try:
            stock_list = await proxy.get_stock_list(
                prefer_source=prefer_source,
                module="test_api"
            )
            list_success = True
    ...
    try:
        ...
        while True:
            ...
            if current_record_count != last_record_count:
                ...
                # 添加各数据源的实时指标
                for source_type, metrics in monitor.source_metrics.items():
                    update_data["data"]["source_metrics"][source_type.value] = {
                        "total_requests": metrics.total_requests,
                        "success_rate": metrics.success_rate,
                        "avg_latency_ms": metrics.avg_latency_ms,
                        "recent_error_rate": metrics.recent_error_rate
