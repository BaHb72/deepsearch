...
async def get_dashboard(request: Request) -> Dict[str, Any]:
    ...
    if not monitor_api:
        ...
        return {
            ...
            "trends": {"events_change": 0, "queue_size_change": 0},
            "event_types": {},
            "alerts": [],
        }

    try:
        return cast(Dict[str, Any], monitor_api.get_dashboard_data())
...
async def get_realtime_metrics(
    request: Request, event_types: Optional[str] = Query(None, description="事件类型列表，逗号分隔")
) -> Dict[str, Any]:
    """
    获取实时指标数据。
    ...
    if not monitor_api:
        # 返回空数据
        return {"time_series": [], "event_types": event_types.split(",") if event_types else []}

    try:
        types = event_types.split(",") if event_types else None
        return cast(Dict[str, Any], monitor_api.get_realtime_metrics(types))
    except Exception as e:
...
async def get_health_status(request: Request) -> Dict[str, Any]:
    ...
    try:
        return cast(Dict[str, Any], monitor_api.get_health_status())
    except Exception as e:
...
async def get_slow_events(
    request: Request, limit: int = Query(50, ge=1, le=1000, description="返回的最大数量")
) -> List[Dict[str, Any]]:
    """
    获取慢事件列表。
    ...
    try:
        return cast(List[Dict[str, Any]], monitor_api.get_slow_events(limit))
    except Exception as e:
...
async def get_historical_data(
    request: Request, hours: int = Query(24, ge=1, le=168, description="历史数据时长（小时）")
) -> Dict[str, Any]:
    """
    获取历史监控数据。
    ...
    try:
        return cast(Dict[str, Any], monitor_api.get_historical_data(hours))
    except Exception as e:
...
async def get_events_summary(request: Request) -> Dict[str, Any]:
    ...
    try:
        metrics = cast(Dict[str, Any], monitor_api.get_realtime_metrics())
        summary = {}
