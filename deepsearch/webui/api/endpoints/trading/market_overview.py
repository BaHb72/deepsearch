...
class MarketDataProvider:
    ...
    async def get_sse_summary(self) -> Dict[str, Any]:
        """获取上海证券交易所市场总貌"""
        try:
    ...
    async def get_szse_summary(self, date: str) -> Dict[str, Any]:
        """获取深圳证券交易所市场总貌"""
        try:
            import akshare as ak
            df = await asyncio.get_event_loop().run_in_executor(
    ...
    async def get_szse_area_summary(self, date: str) -> Dict[str, Any]:
        """获取深交所地区交易排序"""
        try:
            import akshare as ak
            df = await asyncio.get_event_loop().run_in_executor(
    ...
    async def get_szse_sector_summary(self, symbol: str, date: str) -> Dict[str, Any]:
        """获取深交所行业成交数据"""
        try:
            import akshare as ak
            df = await asyncio.get_event_loop().run_in_executor(
    ...
    async def get_sse_deal_daily(self, date: str) -> Dict[str, Any]:
        """获取上交所每日概况"""
        try:
            import akshare as ak
            df = await asyncio.get_event_loop().run_in_executor(
                None, ak.stock_sse_deal_daily, date
            )
            if df is not None and not df.empty:
                return {
                    "success": True,
    ...
    async def get_individual_info(self, symbol: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if df_em is not None and not df_em.empty:
                result["data"]["eastmoney"] = df_em.to_dict('records')
            ...
            try:
                ...
                if df_xq is not None and not df_xq.empty:
                    result["data"]["xueqiu"] = df_xq.to_dict('records')
            except:
                pass
    ...
    async def get_bid_ask(self, symbol: str) -> Dict[str, Any]:
        """获取盘口数据"""
        try:
            import akshare as ak
            df = await asyncio.get_event_loop().run_in_executor(
                None, ak.stock_bid_ask_em, symbol
            )
            if df is not None and not df.empty:
                return {
                    "success": True,
    ...
    async def get_spot_em(self, market: str = "all") -> Dict[str, Any]:
        """获取实时行情数据"""
        try:
            import akshare as ak
...
async def get_szse_summary(
    date: str = Query(..., description="日期，格式：20240619")
):
    """获取深圳证券交易所市场总貌"""
...
async def get_szse_area(
    date: str = Query(..., description="年月，格式：202412")
):
    """获取深交所地区交易排序"""
...
async def get_sse_daily(
    date: str = Query(..., description="日期，格式：20250221")
):
    """获取上交所每日概况"""
...
async def get_spot_data(
    market: str = Query("all", description="市场类型: all/sh/sz/bj/cy/kc/new")
):
    """
    获取实时行情数据
