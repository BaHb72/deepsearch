...
class MarketExchangeSummary(TypedDict, total=False):
...
class MarketOverviewPayload(TypedDict):
    source: str
    exchange: str
    data: dict[str, MarketExchangeSummary]
...
def check_akshare():
    ...
    if not AKSHARE_AVAILABLE:
        raise HTTPException(status_code=503, detail="AKShare数据源未安装或不可用")
...
async def get_akshare_status():
    ...
    try:
        ...
        return APIResponse.success(data=status, message="AKShare状态获取成功")
    except Exception as e:
        logger.error(f"获取AKShare状态失败: {e}")
        return APIResponse.error(
            code=ErrorCodes.INTERNAL_ERROR, message=f"获取状态失败: {str(e)}", status_code=500
        )
...
async def get_sse_summary():
    ...
    try:
        ...
        return APIResponse.success(data=data, message="上交所市场总貌获取成功")
        ...
        return APIResponse.error(
            ...
            status_code=500,
            details={"error_code": "AKSHARE_API_ERROR", "detail": str(e)},
        )


@router.get("/szse/summary")
async def get_szse_summary(date: Optional[str] = Query(None, description="日期，格式：20240110")):
    """
    获取深圳证券交易所市场总貌
    ...
    try:
        ...
        return APIResponse.success(data=data, message="深交所市场总貌获取成功")
        ...
        return APIResponse.error(
            code=ErrorCodes.DATA_SOURCE_ERROR, message=f"获取数据失败: {str(e)}", status_code=500
        )
...
async def get_market_overview(
    ...
    try:
        if source != "akshare":
            return APIResponse.error(
                code=ErrorCodes.DATA_SOURCE_NOT_FOUND,
        ...
        if exchange in ["all", "sse"]:
            try:
                ...
                result["data"]["sse"] = {
                    ...
                    "total_market_value": (
                        float(sse_df[sse_df["项目"] == "总市值"]["股票"].values[0])
                        if len(sse_df) > 0
                        else 0
                    ),
                    "listed_companies": (
                        int(sse_df[sse_df["项目"] == "上市公司"]["股票"].values[0])
                        if len(sse_df) > 0
                        else 0
                    ),
                }
        ...
        if exchange in ["all", "szse"]:
            try:
                ...
                result["data"]["szse"] = {
                    ...
                    "stock_count": (
                        int(szse_df[szse_df["证券类别"] == "股票"]["数量"].values[0])
                        if len(szse_df) > 0
                        else 0
                    ),
                    "total_market_value": (
                        float(szse_df[szse_df["证券类别"] == "股票"]["总市值"].values[0])
                        if len(szse_df) > 0
                        else 0
                    ),
                }
        ...
        return APIResponse.success(data=result, message="市场总貌获取成功")
...
async def get_stock_kline(
    ...
    try:
        ...
        return APIResponse.success(data=data, message=f"股票{symbol} K线数据获取成功")
...
async def get_realtime_quote(symbols: str = Query(..., description="股票代码，多个用逗号分隔")):
    """
    获取实时行情
        ...
        for symbol in symbol_list:
            try:
                ...
                if not stock_data.empty:
                    ...
                    quotes.append({"symbol": symbol, "error": "未找到该股票"})
            except Exception as e:
                quotes.append({"symbol": symbol, "error": str(e)})

        return APIResponse.success(
            data={"quotes": quotes, "count": len(quotes), "update_time": datetime.now()},
            message="实时行情获取成功",
        )
