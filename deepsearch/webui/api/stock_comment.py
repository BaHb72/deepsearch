...
async def get_stock_comment_list(
    ...
    try:
        ...
        if is_cache_valid(cache['stock_comment']):
            df = cache['stock_comment']['data'].copy()
        else:
            # 获取千股千评数据
        ...
        # 搜索过滤
        if search:
            mask = (df['code'].str.contains(search, na=False) | 
        ...
        # 获取统计数据
        top_focus = df.nlargest(5, 'focus_index')[['code', 'name', 'focus_index']].to_dict('records')
        top_score = df.nlargest(5, 'score')[['code', 'name', 'score']].to_dict('records')
        top_institution = df.nlargest(5, 'institution_rate')[['code', 'name', 'institution_rate']].to_dict('records')
...
async def get_stock_detail(symbol: str, period: int = Query(30, ge=7, le=90)):
    ...
    try:
        ...
        # 获取机构参与度
        try:
            df_jgcyd = await run_in_executor(
                ak.stock_comment_detail_zlkp_jgcyd_em, 
                symbol
            )
            df_jgcyd = df_jgcyd.rename(columns={
                '交易日': 'date',
                '机构参与度': 'value'
            })
            # 只取最近period天的数据
        ...
        # 获取历史评分
        try:
            df_score = await run_in_executor(
        ...
        # 获取用户关注指数
        try:
            df_focus = await run_in_executor(
    ...
    try:
        ...
        if not df.empty:
            ...
            if not sh_data.empty:
                result['northFlow'] += sh_data.iloc[0]['资金净流入'] if '资金净流入' in sh_data.columns else 0
                result['shBalance'] = sh_data.iloc[0]['当日资金余额'] if '当日资金余额' in sh_data.columns else 520
            
            if not sz_data.empty:
                result['northFlow'] += sz_data.iloc[0]['资金净流入'] if '资金净流入' in sz_data.columns else 0
                result['szBalance'] = sz_data.iloc[0]['当日资金余额'] if '当日资金余额' in sz_data.columns else 520
            ...
            if not hk_sh_data.empty:
                result['southFlow'] += hk_sh_data.iloc[0]['资金净流入'] if '资金净流入' in hk_sh_data.columns else 0
            
            if not hk_sz_data.empty:
                result['southFlow'] += hk_sz_data.iloc[0]['资金净流入'] if '资金净流入' in hk_sz_data.columns else 0
...
@router.get("/export")
async def export_stock_comment(
