...
def test_get_long_hu_bang():
    ...
    print(f"测试参数:")
    print(f"  股票代码: ['000001.SZ', '600519.SH']")
    ...
    try:
        ...
        if isinstance(data, pd.DataFrame) and not data.empty:
            ...
            # 检查关键字段
            expected_cols = ['MARKET_CODE', 'TRADE_DATE', 'SECURITY_NAME', 'REASON_TYPE_NAME', 
                           'CHANGE_RANGE', 'TRADER_NAME', 'BUY_AMOUNT', 'SELL_AMOUNT']
...
def test_get_block_trading():
    ...
    try:
        ...
        if isinstance(data, pd.DataFrame) and not data.empty:
            ...
            # 检查关键字段
            expected_cols = ['MARKET_CODE', 'TRADE_DATE', 'B_SHARE_PRICE', 'B_SHARE_VOLUME', 
                           'B_FREQUENCY', 'BLOCK_AVG_VOLUME', 'B_SHARE_AMOUNT', 
                           'B_BUYER_NAME', 'B_SELLER_NAME']
            missing_cols = [col for col in expected_cols if col not in data.columns]
            if missing_cols:
                print(f"\n[警告] 缺少字段: {missing_cols}")
...
def test_get_option_basic_info():
    ...
    try:
        data = info.get_option_basic_info(
            test_options,
            local_path=LOCAL_PATH,
            is_local=False
        )
        ...
        if isinstance(data, pd.DataFrame) and not data.empty:
            ...
            # 检查关键字段
            expected_cols = ['CONTRACT_FULL_NAME', 'CONTRACT_TYPE', 'DELIVERY_MONTH', 
                           'EXPIRY_DATE', 'EXERCISE_PRICE', 'EXERCISE_END_DATE',
                           'START_TRADE_DATE', 'LISTING_REF_PRICE', 'LAST_TRADE_DATE',
                           'EXCHANGE_CODE', 'CONTRACT_UNIT', 'MARKET_CODE']
            missing_cols = [col for col in expected_cols if col not in data.columns]
            if missing_cols:
                print(f"\n[警告] 缺少字段: {missing_cols}")
            else:
...
def test_get_option_std_ctr_specs():
    ...
    try:
        data = info.get_option_std_ctr_specs(
            test_codes,
            local_path=LOCAL_PATH,
            is_local=False
        )
        ...
        if isinstance(data, pd.DataFrame) and not data.empty:
            ...
            # 检查关键字段
            expected_cols = ['EXERCISE_DATE', 'CONTRACT_UNIT', 'POSITION_DECLARE_MIN',
                           'QUOTE_CURRENCY_UNIT', 'LAST_TRADING_DATE', 'POSITION_LIMIT',
                           'DELIST_DATE', 'NOTIONAL_VALUE', 'EXERCISE_METHOD',
                           'DELIVERY_METHOD', 'SETTLEMENT_MONTH', 'TRADING_FEE',
                           'EXCHANGE_NAME', 'OPTION_EN_NAME', 'CONTRACT_VALUE',
                           'OPTION_STRIKE_PRICE', 'LISTED_DATE', 'OPTION_NAME',
                           'PREMIUM', 'OPTION_TYPE', 'TRADING_HOURS_DESC']
            missing_cols = [col for col in expected_cols if col not in data.columns]
            if missing_cols:
                print(f"\n[警告] 缺少字段: {missing_cols}")
            else:
