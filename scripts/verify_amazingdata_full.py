...
def verify_amazingdata():
    ...
    try:
        ...
        print(f"  [INFO] 交易日历: {len(calendar)}天")
    except Exception as e:
        print(f"  [WARN] 交易日历获取失败: {str(e)[:30]}")

    # K线数据
    try:
        market = ad.MarketData(calendar)  # 传入calendar
        data = market.query_kline(
            ["000001.SZ"], begin_date=20241101, end_date=20241130, period=10008
        )  # Period.day.value
        if data and "000001.SZ" in data:
            records = data["000001.SZ"]
    ...
    # 快照数据
    try:
        market = ad.MarketData(calendar)  # 复用calendar
        data = market.query_snapshot(["000001.SZ"])
        if data and "000001.SZ" in data:
            record("快照数据", True, 1)
        else:
    ...
    print("\n分类统计:")
    print("  BaseData: 基础数据接口")
    print("  MarketData: 行情数据接口")
    print("  InfoData: 资讯/财务数据接口")
