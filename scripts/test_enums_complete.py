...
def test_div_progress_enum():
    ...
    for prog in div_progresses:
        print(f"枚举: {prog.name:35s} 数值: {prog.value:2d} 说明: {get_div_progress_name(prog.value)}")
...
def test_extended_statement_types():
    ...
    for stmt_type in statement_types:
        print(f"枚举: {stmt_type.name:20s} 数值: {stmt_type.value:2d} 说明: {get_statement_type_name(stmt_type.value)}")
...
def test_all_enums_count():
    ...
    total = (len(AmazingDataTradingPhase) + 
...
def test_progress_filtering():
    ...
    for div in mock_dividends:
        print(f"  {div['stock']}: 日期={div['div_date']}, "
