...
这是一个独立的测试脚本，只测试枚举类型定义，不依赖AmazingData SDK
"""
...
def test_trading_phase_enum():
    ...
    for phase in phases:
        print(
            f"枚举: {phase.name:40s} 代码: {phase.value:3s} 说明: {get_trading_phase_name(phase.value)}"
        )
...
def test_report_period_enum():
    ...
    for period in periods:
        print(
            f"枚举: {period.name:10s} 数值: {period.value} 说明: {get_report_period_name(period.value)}"
        )
...
def test_statement_type_enum():
    ...
    for stmt_type in statement_types:
        print(
            f"枚举: {stmt_type.name:35s} 数值: {stmt_type.value:2d} 说明: {get_statement_type_name(stmt_type.value)}"
        )
...
def test_enum_in_filter():
    ...
    for quote in mock_quotes:
        print(
            f"  {quote['code']}: 价格={quote['last_price']}, 状态={quote['trading_phase_code']} ({get_trading_phase_name(quote['trading_phase_code'])})"
        )
