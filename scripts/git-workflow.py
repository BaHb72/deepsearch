...
class GitWorkflow:
    ...
    def create_hotfix(self, name: str):
        ...
        if code == 0:
            print(f"✅ 已创建并切换到 {branch_name}")
            print("⚠️ 注意: 紧急修复完成后需要同时合并到master和dev分支")
        else:
            print(f"❌ 创建分支失败")
    ...
    def cleanup_branches(self):
        ...
        branches_to_delete = [b for b in branches
                             if not any(p in b for p in protected)]
