...
async def diagnose():
    ...
    try:
        from deepsearch.infrastructure.providers.implementations.amazingdata._sdk_loader import (
            ad, HAS_AMAZINGDATA, IMPORT_ERROR
        )
    ...
    try:
        ...
        # 构建配置
        connection_cfg = getattr(raw_config, 'connection', {}) if hasattr(raw_config, 'connection') else raw_config.get('connection', {})
    ...
    if 'provider' in dir() and provider._connected:
        try:
            ...
            result = await asyncio.wait_for(
                provider.get_balance_sheet(['SH.600519']),
                timeout=30.0
            )
            elapsed = time.time() - start_time
