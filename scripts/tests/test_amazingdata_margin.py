...
async def run_margin_tests():
    """运行融资融券API测试"""
    print("\n" + "=" * 70)
    print("AmazingData 融资融券 API 测试")
    print("=" * 70)
    ...
    # ==================== 1. get_margin_summary ====================
    async def test_get_margin_summary():
        data = await provider.get_margin_summary()
        return data

    results.append(await test_api("get_margin_summary (融资融券汇总)", test_get_margin_summary))

    # ==================== 2. get_margin_detail ====================
    async def test_get_margin_detail():
        data = await provider.get_margin_detail(code_list=test_codes)
        return data

    results.append(await test_api("get_margin_detail (融资融券明细)", test_get_margin_detail))

    # ==================== 3. get_long_hu_bang ====================
    async def test_get_long_hu_bang():
        data = await provider.get_long_hu_bang(code_list=test_codes)
        return data

    results.append(await test_api("get_long_hu_bang (龙虎榜)", test_get_long_hu_bang))

    # ==================== 4. get_block_trading ====================
    async def test_get_block_trading():
        if hasattr(provider, "get_block_trading"):
            data = await provider.get_block_trading(code_list=test_codes)
            return data
        else:
            return {"success": False, "data": None, "error": "方法不存在"}

    results.append(await test_api("get_block_trading (大宗交易)", test_get_block_trading))
