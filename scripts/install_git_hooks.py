    ...
    def install_pre_commit(self):
        ...
        try:
            ...
            if result.returncode == 0:
                print(f"  ✅ pre-commit已安装: {result.stdout.strip()}")
            else:
...
if [ -n "$current_coverage" ]; then
    if (( $(echo "$current_coverage < $coverage_threshold" | bc -l) )); then
        echo "⚠️  警告：代码覆盖率 ${current_coverage}% 低于阈值 ${coverage_threshold}%"
    ...
    try:
        with open(filepath, 'r', encoding='gbk') as f:
            ...
            if not first_line.startswith('# encoding:gbk'):
                print(f"❌ {filepath}: 缺少编码声明")
                return False
        return True
    except UnicodeDecodeError:
        print(f"❌ {filepath}: 不是GBK编码")
        return False
...
'''
        ...
        print("  ✅ check_qmt_encoding.py 创建成功")
    ...
    try:
        ...
        for field in required_fields:
            if field not in config:
                print(f"❌ {filepath}: 缺少必要字段 '{field}'")
                return False

        return True
    except Exception as e:
        print(f"❌ {filepath}: 配置文件错误 - {e}")
        return False
