...
from core.config.crypto import decrypt_password, encrypt_password
from core.config.services.database_connections import (
    load_database_connections,
    persist_database_connections,
)
from core.core.interfaces.component import Component, ComponentStatus
from core.core.runtime.context import get_context
from core.infrastructure.persistence.duckdb_path import resolve_duckdb_path
from core.infrastructure.persistence.runtime_state.database_status_store import (
...
from apps.api.api.database_states import (
...
async def deactivate_connection(
    ...
    try:
        ...
        if options.disconnect and (connection.type or "").lower() in {
            ...
            try:
                from apps.api.api.database import disconnect_database
...
async def _attempt_immediate_connection(
    ...
    if conn_type in {"postgresql", "mysql", "sqlite"}:
        from apps.api.api.database import ConnectRequest, connect_database
