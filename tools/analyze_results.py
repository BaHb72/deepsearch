...
def analyze_results():
    ...
    if 'service' in files_by_type:
        ...
        service_complexities = [(f, c) for f, c in zip(files_by_type['service'], 
                                                       complexity_by_type['service'])]
...
""")
    ...
    migration_plan = {
        ...
        'priorities': {
            'p1_domain': [f for f in files_by_type.get('entity', []) + files_by_type.get('model', [])][:20],
            'p2_services': [f for f in files_by_type.get('service', [])][:20],
            'p3_infrastructure': [f for f in files_by_type.get('repository', []) + files_by_type.get('provider', [])][:20],
            'p4_interfaces': [f for f in files_by_type.get('controller', []) + files_by_type.get('api', [])][:20]
        }
    }
