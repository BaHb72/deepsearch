...
class MigrationExecutor:
    ...
    def _migrate_file(self, candidate: MigrationCandidate):
        ...
        print(f"{'[DRY RUN] ' if self.dry_run else ''}Migrating {candidate.source_path} -> {candidate.target_path}")
        
        if not self.dry_run:
            # Create backup
            backup_path = self.backup_dir / candidate.source_path.relative_to(candidate.source_path.parent.parent)
...
def main():
    ...
    parser.add_argument('action', choices=['analyze', 'plan', 'migrate', 'rollback'],
                       help='Action to perform')
    parser.add_argument('--source', type=str, default='deepsearch',
                       help='Source directory')
    parser.add_argument('--target', type=str, default='deepsearch',
                       help='Target directory')
    parser.add_argument('--dry-run', action='store_true',
                       help='Perform dry run without actual changes')
    parser.add_argument('--plan-file', type=str, default='migration_plan.json',
                       help='Migration plan file')
    ...
    if args.action == 'analyze':
        ...
        for item in plan_data:
            plan.append(MigrationCandidate(
