...
class HealthMetric:
    ...
    def get_status_emoji(self) -> str:
        ...
        return {"good": "[GOOD]", "warning": "[WARNING]", "critical": "[CRITICAL]"}.get(
            self.status, "[UNKNOWN]"
        )
...
class ArchitectureHealthMonitor:
    ...
    def _check_code_metrics(self) -> List[HealthMetric]:
        ...
        metrics.append(
            HealthMetric(
                ...
                status=(
                    "good"
                    if avg_complexity <= self.targets["avg_complexity"]
                    else (
                        "warning"
                        if avg_complexity <= self.targets["avg_complexity"] * 1.5
                        else "critical"
                    )
                ),
            )
        ...
        metrics.append(
            HealthMetric(
                ...
                status=(
                    "good"
                    if max_lines <= self.targets["max_file_size"]
                    else "warning" if max_lines <= self.targets["max_file_size"] * 2 else "critical"
                ),
            )
        )
        ...
        metrics.append(
            HealthMetric(
                ...
                status=(
                    ...
                    else "warning" if cohesion_score >= 60 else "critical"
                ),
            )
        )
    ...
    def _check_performance_metrics(self) -> List[HealthMetric]:
        ...
        metrics.append(
            HealthMetric(
                ...
                status=(
                    "good"
                    if memory_mb <= self.targets["memory_usage"]
                    else (
        ...
        metrics.append(
            HealthMetric(
                ...
                unit="%",
                status=(
                    "good" if cpu_percent <= 50 else "warning" if cpu_percent <= 75 else "critical"
                ),
            )
        )
    ...
    def _check_quality_metrics(self) -> List[HealthMetric]:
        ...
        try:
            result = subprocess.run(
                ["python", "-m", "pytest", "--co", "-q"], capture_output=True, text=True, timeout=10
            )
        ...
        metrics.append(
            HealthMetric(
                ...
                status=(
                    "good"
                    if estimated_coverage >= self.targets["test_coverage"]
                    else "warning" if estimated_coverage >= 60 else "critical"
                ),
            )
        )
        ...
        metrics.append(
            HealthMetric(
                ...
                status=(
                    "good"
                    if doc_coverage >= 80
                    else "warning" if doc_coverage >= 50 else "critical"
                ),
            )
        )
    ...
    def _analyze_issues(self, metrics: List[HealthMetric]) -> List[str]:
        ...
        for metric in metrics:
            if metric.status == "critical":
                issues.append(
                    f"CRITICAL: {metric.name} is {metric.value:.1f} (target: {metric.target})"
                )
            elif metric.status == "warning":
                issues.append(
                    f"WARNING: {metric.name} is {metric.value:.1f} (target: {metric.target})"
                )
    ...
    def _generate_recommendations(self, metrics: List[HealthMetric]) -> List[str]:
        ...
        if hexagonal and hexagonal.value < hexagonal.target:
            ...
            recommendations.append(
                f"Complete hexagonal architecture setup ({missing} layers missing)"
            )
        ...
        if test_coverage and test_coverage.value < test_coverage.target:
            ...
            recommendations.append(
                f"Increase test coverage by {gap:.0f}% - focus on critical paths"
            )
    ...
    def generate_report(self, health: ArchitectureHealth) -> str:
        ...
        report.append(
            f"Overall Score: {health.overall_score:.1f}/100 ({health.get_status().upper()})"
        )
        report.append("")
        ...
        if len(self.history) > 1:
            ...
            if score_change > 0:
                ...
                report.append("  ➡️ Score unchanged")
    ...
    def create_dashboard_data(self) -> Dict[str, Any]:
        ...
        dashboard_data = {
            ...
            "metrics": {
                m.name: {"value": m.value, "target": m.target, "score": m.score, "status": m.status}
                for m in latest.metrics
            },
            "history": [
                {"timestamp": h.timestamp.isoformat(), "score": h.overall_score}
        ...
        if health_reports.exists():
            ...
            for report_file in reports[-10:]:  # Last 10
                with open(report_file) as f:
                    data = json.load(f)
                print(
                    f"{data['timestamp']}: Score={data['overall_score']:.1f}, Status={data['status']}"
                )
    else:
