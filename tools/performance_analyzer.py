...
class PerformanceAnalyzer:
            ...
            for py_file in path.rglob("*.py"):
                ...
                try:
                    with open(py_file, "r", encoding="utf-8") as f:
                        ...
                        if complexity > 10:
                            complex_files.append({"file": str(py_file), "complexity": complexity})

                except Exception:
                    pass

        self.results["code_complexity"] = {
            "complex_files": sorted(complex_files, key=lambda x: x["complexity"], reverse=True)[
                :10
            ],
    ...
    def generate_optimization_suggestions(self):
        ...
        slow_apis = [
            api
            for api, data in self.results["api_response_times"].items()
            if data.get("status") == "SLOW"
        ]
    ...
    def save_report(self):
        ...
        print("\n[*] 报告已保存到:")
        print("  - performance_analysis_report.json")
