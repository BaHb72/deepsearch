    ...
    async def analyze_latency_bottlenecks(self) -> List[Bottleneck]:
        ...
        for source, latencies in source_latencies.items():
            ...
            if p95_latency > 200:  # P95延迟超过200ms
                severity = (
                    "critical" if p95_latency > 500 else "high" if p95_latency > 300 else "medium"
                )
                ...
                bottlenecks.append(
                    Bottleneck(
                        ...
                        code_snippet=self._generate_latency_optimization_code(
                            source, avg_latency, p95_latency
                        ),
                    )
                )
                ...
                for error_type, count in by_type.items():
                    ...
                    if error_rate > 0.2:  # 某类错误占比超过20%
                        severity = (
                            "critical"
                            if error_rate > 0.5
                            else "high" if error_rate > 0.3 else "medium"
                        )
...
'''
    ...
    def print_summary(self):
        ...
        for metric in self.metrics[:10]:
            ...
            print(
                f"  {status_icon} {metric.name:30} {metric.value:10.2f} {metric.unit:5} "
        ...
        for i, bottleneck in enumerate(self.bottlenecks[:5], 1):
            ...
            print(
                f"     当前值: {bottleneck.current_value:.1f} | 期望值: {bottleneck.expected_value:.1f}"
            )
...
async def main():
    ...
    # 保存报告
    report_file = (
        f"./data/monitoring/performance_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
    ...
    if analyzer.bottlenecks:
        code_file = (
            f"./data/monitoring/optimization_code_{datetime.now().strftime('%Y%m%d_%H%M%S')}.py"
        )
