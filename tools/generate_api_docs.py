...
def scan_frontend_apis() -> Dict[str, List[Dict]]:
    ...
    for api_file in frontend_api_dir.glob("*.js"):
        ...
        # 匹配export function
        func_pattern = r'export\s+(?:async\s+)?function\s+(\w+)\s*\([^)]*\)\s*{[^}]*?(?:url|URL)\s*:\s*[\'"`]([^\'"`]+)[\'"`][^}]*?(?:method|METHOD)\s*:\s*[\'"`](\w+)[\'"`]'
        
        # 更宽松的匹配模式
        simple_pattern = r'(?:url|URL)\s*:\s*[\'"`]([^\'"`]+)[\'"`].*?(?:method|METHOD)\s*:\s*[\'"`](\w+)[\'"`]'
...
def generate_mapping_doc(frontend_apis: Dict, backend_apis: Dict) -> str:
    ...
    for module, module_apis in sorted(frontend_apis.items()):
        for api in module_apis:
            ...
            doc.append(f"| {module} | {api['function']} | {api['url']} | {api['method']} | {backend_module} | {backend_func} | {status} |\n")
    ...
    if unmatched_frontend:
        ...
        for item in unmatched_frontend:
            doc.append(f"- **{item['module']}.{item['function']}**: `{item['method']} {item['path']}`\n")
