...
class AkShareAPIValidator:
    ...
    def discover_apis(self) -> Dict[str, List[str]]:
        ...
        for member_name in all_members:
            ...
            if member_name.startswith("stock_"):
                ...
                # 其他可能的API函数
                if any(keyword in member_name for keyword in 
                       ["get", "fetch", "download", "query", "search", "find"]):
    ...
    def generate_report(self) -> str:
        ...
        report_lines.append(f"**Test Duration**: {(datetime.now() - self.start_time).total_seconds():.1f} seconds\n")
        ...
        report_lines.append("| Category | Total | Success | Failed | Empty | Timeout | Success Rate |")
        report_lines.append("|----------|-------|---------|--------|-------|---------|--------------|")
        ...
        successful_apis = [r for r in self.results if r["status"] == "success" and r["response_time"]]
        if successful_apis:
            ...
            report_lines.append(f"- **Fastest Function**: `{fastest['function_name']}` ({fastest['response_time']:.3f}s)")
            report_lines.append(f"- **Slowest Function**: `{slowest['function_name']}` ({slowest['response_time']:.3f}s)")
        ...
        for category in ["stock", "index", "fund", "macro"]:
            category_success = [
                ...
                if r["category"] == category and r["status"] == "success"
            ]
        ...
        # 建议和注意事项
        report_lines.append("\n## Recommendations\n")
        report_lines.append("1. **Use cached data** for frequently accessed APIs to reduce server load")
    ...
    def save_report(self, filename: str = None):
        """保存报告到文件"""
        if filename is None:
            filename = f"AKSHARE_API_VALIDATION_REPORT_{datetime.now().strftime('%Y%m%d_%H%M%S')}.md"
...
def main():
    ...
    if validator.results:
        print(f"Success rate: {sum(1 for r in validator.results if r['status'] == 'success') / len(validator.results) * 100:.1f}%")
